...
func _initialize() -> void:
	...
	if output.is_empty() and not status_only:
		printerr("Usage: Godot --headless --path presentation-playground --script res://games/bishoujo_afterlight/voice/export_inventory.gd -- --output /absolute/path/inventory.json [--status]")
		quit(2)
		return
	var text_sets := {}
