...
const CAPTURE_DIRECTORY := "res://qa/afterlight-ensemble"
...
func _language_and_detour(game: Control, id: String) -> void:
	...
	_expect(_app.open_route("game:presentation_lab"), "The dedicated laboratory must be reachable from the episode.")
	await _settle()
	_expect(_app.selected_game_id == "presentation_lab", "Technical demonstrations must live in the laboratory game.")
	_expect(_app.open_route("game:bishoujo_afterlight"), "The episode must resume after the laboratory detour.")
	await _settle()
