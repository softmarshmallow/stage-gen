...
const PAN_OUTPUT := "res://qa/cast-pan"
...
func _run() -> void:
	...
	var sources := {}
	for source: String in ["res://addons/game_presentation/motion/layer_pan.gd", "res://games/bishoujo_afterlight/cast_stage.gd", "res://games/bishoujo_afterlight/root.gd", "res://games/bishoujo_afterlight/story.gd", "res://games/bishoujo_afterlight/story_beats.gd", "res://games/presentation_lab/afterlight/cast_pan_study.gd"]:
...
func _laboratory_pan(factor: int) -> void:
	_expect(_app.open_route("game:presentation_lab/cast_pan_study"), "Cast Pan must have its dedicated laboratory route.")
	await _settle()
	var lab := _active()
	lab.set_language("ko")
	...
	_expect(_app.open_route("game:bishoujo_afterlight"), "The laboratory must return to the saved story.")
	await _settle()
