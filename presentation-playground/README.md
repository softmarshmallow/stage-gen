...
The development folder remains `presentation-playground/` so existing launch
...
Start a new game with the [standalone starter](starter_source/README.md), whose
...
local content root; see [packaging and external content](PACKAGING.md).
...
The shared [Point Contact](presentation/interaction/README.md) behavior serves
...
[Afterlight voice contract](games/bishoujo_afterlight/voice/README.md) owns policy,
...
- [Afterlight play, story, and effects](games/bishoujo_afterlight/README.md)
- [Paired English/Korean story review](games/bishoujo_afterlight/text/STORY_REVIEW.md)
- [Presentation Lab](games/presentation_lab/README.md)
- [Topology and responsibilities](TOPOLOGY.md)
- [Presentation terminology dictionary](TERMINOLOGY.md)
...
are in [Character visual effects](CHARACTER_EFFECTS.md).
...
- Command Link has two 1080p anime opening edits, 22.5 and 22 seconds, with one Begin Briefing
  control and a fade through black between loops. [Opening behavior](assets/opening/README.md).
...
[the tactical UI notes](presentation/ui/README.md). Body text uses Godot's bundled font;
...
```sh
Godot --editor --path presentation-playground
```

To play directly:

```sh
Godot --path presentation-playground
```
...
```sh
Godot --path presentation-playground -- --route demos/contact
```
...
[Point Contact](presentation/interaction/README.md). That behavior contains no
...
P99 adds [Sprite Particle Emitter](presentation/effects/SPRITE_PARTICLE_EMITTER.md)
under Particle Effects and [Transmission Voice](presentation/audio/VOICE_PROCESSING.md)
...
[Radial Sprite Burst](presentation/effects/SPRITE_BURST.md) keeps motion and
...
P92 adds [Stepped Transform Loops and Sprite-Frame Loops](presentation/manpu/README.md).
...
The [Manpu Introduction contract](presentation/manpu/README.md) and
[shared Presentation Animation contract](presentation/animation/README.md) describe the
...
[Establishing Shot contract](presentation/camera/README.md). Coastal Staging's flare follows
...
[Dialogue Camera contract](presentation/camera/DIALOGUE_CAMERA.md) records cue ownership,
...
The spike-local [Actor Focus contract](presentation/focus/README.md) separates the
...
[Presentation Animation sampler](presentation/animation/README.md). Adding a preset using
...
The [Character Exit contract](presentation/transitions/README.md) uses the existing texture
...
The [Cast Transition contract](presentation/transitions/CAST_TRANSITIONS.md) records the
...
speaker modulation. [CHARACTER_EFFECTS.md](CHARACTER_EFFECTS.md) explains
...
```sh
Godot --headless --path presentation-playground -- --validate-routes
```

Capture the new playable and demo routes in a windowed run with:

```sh
Godot --path presentation-playground -- --capture-routes
```
...
```sh
Godot --headless --path presentation-playground -- --validate
```
...
```sh
Godot --path presentation-playground -- --capture-all
```
