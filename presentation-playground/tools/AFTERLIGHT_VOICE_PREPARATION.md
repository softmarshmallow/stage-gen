...
```sh
/Users/universe/.local/bin/Godot --headless --path presentation-playground --script res://games/bishoujo_afterlight/voice/export_inventory.gd -- --output /tmp/afterlight-voice-inventory.json
```

Inspect preparation and cost without credentials, network access, or writes:

```sh
.venv/bin/python presentation-playground/tools/prepare_afterlight_voice.py --inventory /tmp/afterlight-voice-inventory.json --quote presentation-playground/art/voiceovers-p95/quote.json
```
...
```sh
.venv/bin/python presentation-playground/tools/prepare_afterlight_voice.py --inventory presentation-playground/art/voiceovers-p95/inventory.json --quote presentation-playground/art/voiceovers-p95/quote.json --live --yes
```
...
```sh
.venv/bin/python -m pytest -q presentation-playground/qa/python/test_afterlight_voice_preparation.py
```
