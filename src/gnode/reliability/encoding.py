...
#:
#: ``video`` is here for the same reason ``image`` and ``audio`` are: it has a
#: ring-1 service that owns its retries, so its byte floor is a signature check
#: inside that retry owner rather than the deterministic write-time check
#: ``text`` gets below. Nothing in ring 0 decodes any of them.
