...
from gnode.modalities.signatures import assert_video_signature
...
class VideoGenerationService:
    """The one retry owner for a video-generation route.

    Transport, the video-signature floor, and the caller's validator all run
    inside the single bounded attempt loop, so a clip that came back as a still,
    ran to the wrong length, or arrived as a container the host cannot open is a
    retried attempt and never a persisted artifact.

    A video attempt is minutes rather than seconds, so callers are expected to
    set ``timeout_seconds`` well above the default per-attempt deadline - and to
    raise their scheduler's own node timeout to match, or the node dies on the
    scheduler's clock and the failure history dies with it.
    """

    def __init__(
        self,
        backend: VideoModelV1,
        *,
    ...
    async def generate(self, request: VideoGenerationRequest) -> VideoGenerationResult:
        ...
        async def attempt(context: RetryContext) -> tuple[ProviderVideo, dict[str, object]]:
            nonlocal attempts
            attempts = context.attempt
            generated = await self._backend.generate_once(request)
            assert_video_signature(generated.data, generated.media_type)
        ...
        generated, caller_facts = await retry_with_backoff(
            ...
            label=f"{self._backend.provider} video generation",
