...
class SoundEffectGenerationService:
    """The one retry owner for a text-to-sound-effect route.

    Transport, the audio-signature floor, and the caller's validator all run
    inside the single bounded attempt loop, so a silent or clipped draw is a
    retried attempt and never a persisted artifact.
    """

    def __init__(
        self,
        backend: SoundEffectModelV1,
        *,
    ...
    async def generate(self, request: SoundEffectGenerationRequest) -> SoundEffectGenerationResult:
        attempts = 0

        async def attempt(
            context: RetryContext,
        ) -> tuple[ProviderSoundEffect, dict[str, object]]:
            nonlocal attempts
        ...
        generated, caller_facts = await retry_with_backoff(
            ...
            label=f"{self._backend.provider} sound effect generation",
