...
class ScrollingPreviewExecutor:
    ...
    async def _generate_image_asset(
        ...
        if not force and style is None:
            ...
            if resume_record is not None:
                return await self._generate_per_cell_grid(
                    context,
                    spec,
                    sheet_failures=cast(list[dict[str, object]], resume_record["sheet_failures"]),
    ...
    async def _derive_transparency(
        ...
        if isolated_view_contract is not None and str(mode) in {
            ...
            if isolated_view_fit is not None:
                processor = f"{processor}+{ISOLATED_SUBJECT_FIT_VERSION}"
        if grid_contract is not None:
            processor = f"{processor}+grid-cell-normalization"
        processor_version = (
            GRID_NORMALIZATION_VERSION
            if grid_contract is not None
...
def _asset_kind_for_image_stage(stage: str) -> ImageAssetKind:
    ...
    if stage in {"items", "ladder", "village-fixtures"} or stage.startswith("obstacles-"):
        return "asset_sheet"
    if stage == "inventory":
...
def _effective_image_prompt(
    ...
    The exact duplication is deliberate: cache identity must change whenever generation text
    changes, while accepted artifacts must remain reusable when the effective prompt is stable.
    """
