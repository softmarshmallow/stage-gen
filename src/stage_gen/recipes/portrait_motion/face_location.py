...
def _implementation_digest() -> str:
    ...
    files["stage_gen/recipes/portrait_motion/face_location.py"] = sha256_hex(
        ...
        try:
            ...
            if live and not cached:
                if service_factory is None:
                    raise ValueError("Live face location requires an injected service_factory")
                service = service_factory.locator(store, plan, dotenv)
                owned = True
