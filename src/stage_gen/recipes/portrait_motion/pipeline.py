"""Portrait asset recipe: immutable planning, bounded stages, receipts, and recovery."""
...
from stage_gen.portrait_policy import request_policy as request_policy
...
def prepare_run(
    ...
    *,
    ...
    if face_crop:
        from .face import prepare_face_run
...
async def run_pipeline(
    ...
    *,
    ...
    """
    from .face import is_face_run, run_face_pipeline
    ...
    with _run_lock(store):
        ...
        if live:
            ...
            bundle = service_factory.motion(store, plan, graph, dotenv)
            image_service, structured_service = bundle.image_service, bundle.structured_service
            owned = True
        host = _host(store, plan, graph, image_service, structured_service)
        if bundle is not None:
            host.operation_count = bundle.operation_count
...
def verify_run(run_dir: Path) -> dict[str, Any]:
    from .face import is_face_run, verify_face_run
