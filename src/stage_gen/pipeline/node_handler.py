...
from stage_gen.pipeline.route_context import node_route_context
...
class CachedNodeHandler(ABC):
    ...
    async def __call__(self, node: Node, context: NodeExecutionContext) -> NodeExecutionResult:
        ...
        try:
            ...
            # Publication already completed. Cache persistence cannot erase the
            # operations and cost incurred by the successful handler invocation.
            # OSError.__str__ can include private cache filenames. Preserve the
            # portable failure reason and accounting, keeping paths in the cause.
            ...
            failure = NodeExecutionError(
                f"node cache publication failed: {detail}",
