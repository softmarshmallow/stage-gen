"""Cancellation-aware all-or-nothing persistence for loop artifact sets."""
...
async def persist_loop_files(
    files: Sequence[PendingLoopFile],
    *,
    cancellation: CancellationToken | None,
    secrets: tuple[str, ...],
    checkpoint: PersistenceCheckpoint | None = None,
) -> None:
    """Install all files or none, including cancellation during filesystem work."""
...
def _persist_loop_files(
    files: tuple[PendingLoopFile, ...],
    cancellation: CancellationToken | None,
    task_cancelled: threading.Event,
    checkpoint: PersistenceCheckpoint | None,
) -> None:
    if not files:
        raise ValueError("loop persistence requires at least one file")
    parents = {item.path.parent.resolve() for item in files}
    if len(parents) != 1:
        raise ValueError("loop persistence targets must share one output directory")
    parent = next(iter(parents))
    parent.mkdir(parents=True, exist_ok=True)
    temporary: list[tuple[PendingLoopFile, Path]] = []
    installed: list[PendingLoopFile] = []
    try:
        _raise_if_cancelled(cancellation, task_cancelled)
        for item in files:
            _raise_if_cancelled(cancellation, task_cancelled)
            temp = parent / f".{item.path.name}.{uuid.uuid4().hex}.loop-tmp"
...
def _raise_if_cancelled(
    ...
    if task_cancelled.is_set():
        raise _TaskPersistenceCancelled("task cancelled during loop persistence")
    if cancellation is not None:
        cancellation.raise_if_cancelled()


def _remove_committed(files: tuple[PendingLoopFile, ...]) -> None:
