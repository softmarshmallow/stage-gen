...
**Artifact media families.** A persisted artifact declares one of five
families — `image`, `audio`, `application`, `text`, `video` — and ring 0 refuses any
other by name at provenance build time. `text` (`text/plain`,
`text/markdown`) exists because prose records are written by callers, not
generated by a modality; having no ring-1 service to guard them, their
deterministic floor is enforced by the write itself: the bytes must decode as
...
| `VideoModelV1` | video generation with optional image references | `generate_once` |
