...
These names are compatibility-stable for v1. “Visual Content Direction” is the
capability name; no renamed wire field or public Python facade is implied.

## Stage graph

When `theme` is absent, the recipe returns its exact historical six-stage
...
| `hard_exclusions` | Affirmative observable baseline binding every downstream asset. Despite the historical field name, it is not a negative-prompt list. |
...
retains the historical prompt composition byte-for-byte.
