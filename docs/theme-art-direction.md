...
plain prompt keeps the historical scrolling-preview path. They are currently
scoped to `scrolling-preview`.
...
TOML or JSON brief + handles
  ...
  -> structured theme compiler + tracked art-direction skill
...
For a themed run, the original brief and normalized handles stop at that
compiler boundary. The compiler artifact and sidecar retain the exact source
brief and canonical control record for auditability, but the downstream world
planner receives only validated `concept`, `world_spec`, and binding-baseline
prose plus the compiled concept image. Image requests likewise receive only the
validated plan field selected for that asset and deterministic recipe
instructions. An unthemed run keeps the historical prompt path unchanged.
...
- Deterministic validation catches known cue combinations within each field and
  across the joined seven-field plan, but every fresh plan still needs
  independent semantic review before a live image call.
