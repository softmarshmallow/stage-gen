...
Visual Content Direction is the capability name. Its one current v1 contract
uses these exact identifiers:
...
These identifiers are not aliases and do not claim that the feature controls
artistic style or narrative theme. `[content_controls]`, a direct compiler
command, and alternative Python contracts are rejected or unsupported rather
than treated as another schema generation.
...
Omitting `theme` preserves the exact baseline stage graph and prompt path. A
...
7. preserve its baseline path exactly when controls are absent.
