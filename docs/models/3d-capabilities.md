...
| Tripo P1 / H3.1 | Text/image; H3.1 also multiview | fal has versioned [P1](https://fal.ai/models/tripo3d/p1/image-to-3d/api) and [H3.1](https://fal.ai/models/tripo3d/h3.1/image-to-3d/api) routes. P2 and direct mesh-processing parity on fal were not confirmed. Old v2.5 routes are not P2 aliases. |
...
| Rodin Gen-2 Edit | Existing model + selected region + prompt → local geometry edit; [vendor explanation](https://hyper3d.ai/blog/rodin-gen-2). | App/product documented. Public [API index](https://docs.hyper3d.ai/en/api-specification/rodin-gen2-5) lists generation, Bang and texture; a prompted geometry-edit endpoint was not established. Exact unchanged-region, UV and rig preservation need a trial. |
...
| Tripo Segment + Complete | External mesh + optional semantic labels/reference segmentation mask → parts; segmentation task → completed parts. [Segment](https://developers.tripo3d.ai/en/docs/mesh-segment), [complete](https://developers.tripo3d.ai/en/docs/mesh-complete). | v2 segmentation adds controls; completion offers generated hidden surfaces or quick boundary caps. Neither is a command to weld a neck while preserving its rig. |
| Rodin Bang | Model → separated components; [direct API](https://docs.hyper3d.ai/en/api-specification/bang). | Decomposition is not a guarantee of anatomical labels or continuous deforming joints. Check current input provenance restrictions before selecting it for an external asset. |
