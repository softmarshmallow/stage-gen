...
export function movementCapabilities(
  ...
  const merged = { ...DEFAULT_MOVEMENT_CAPABILITIES, ...overrides };
  for (const value of [
    merged.walkSpeed,
    merged.runSpeed,
    merged.jumpVelocity,
...
function jumpMoveFor(
  ...
  if (simulatePlatformJump({ ...shared, airJumpVelocity: null }).reachable) return "jump";
  if (capabilities.airJumpVelocity === null) return null;
  return simulatePlatformJump({ ...shared, airJumpVelocity: capabilities.airJumpVelocity })
    .reachable
    ? "double_jump"
    : null;
}
...
/**
 * Cut the terrain into level lanes.
 *
 * A lane is a maximal run of columns sharing one surface, because that is precisely the run the
 * controller walks without leaving the ground: `resolveTerrainWalk` treats any column standing
 * above the foot as a wall, and the tolerance for "above" is a single unit. Slopes therefore do
 * not exist here — a terrain of stepped tiles is a stack of level shelves, which is what it is.
 */
function terrainLanes(
  columnSurfaceY: readonly number[],
  tileUnits: number,
  tolerance: number,
): NavNode[] {
  const lanes: NavNode[] = [];
  let start = 0;
  for (let column = 1; column <= columnSurfaceY.length; column += 1) {
    const previous = columnSurfaceY[column - 1]!;
    const current = column < columnSurfaceY.length ? columnSurfaceY[column] : undefined;
    if (current !== undefined && Math.abs(current - previous) <= tolerance) continue;
    lanes.push(
      Object.freeze({
        id: `terrain:${lanes.length}`,
        kind: "terrain" as const,
        left: start * tileUnits,
        right: column * tileUnits,
        surfaceY: columnSurfaceY[start]!,
      }),
    );
    start = column;
  }
...
export function buildNavGraph(input: NavGraphInput): NavGraph {
  ...
  const lanes = terrainLanes(input.columnSurfaceY, input.tileUnits, capabilities.stepUpTolerance);
...
function walkToward(
  ...
  extra: Partial<PlayerIntent> = {},
): PlayerIntent {
  const delta = targetX - self.x;
  if (Math.abs(delta) <= tuning.arriveRadiusUnits) return playerIntent(extra);
  return playerIntent({
    ...extra,
    left: delta < 0,
    right: delta > 0,
...
export function steerNav(input: Readonly<{
  ...
  if (!link) return walkToward(self, input.targetX, tuning);
  switch (link.move) {
    case "walk":
    case "step_down":
      return walkToward(self, link.toX, tuning);
    case "jump":
    case "double_jump": {
      if (self.airborne) {
        const spendAirJump =
        ...
        return walkToward(self, link.toX, { ...tuning, arriveRadiusUnits: 0 }, {
          jump: spendAirJump,
        });
      }
      const atLaunch = Math.abs(self.x - link.fromX) <= tuning.launchWindowUnits;
      return walkToward(self, atLaunch ? link.toX : link.fromX, { ...tuning, arriveRadiusUnits: 0 }, {
        jump: atLaunch,
      });
    }
    case "climb": {
      ...
      if (self.support === "climbable") {
        return playerIntent({ up: upward, down: !upward });
      }
      if (!aligned) return walkToward(self, link.fromX, { ...tuning, arriveRadiusUnits: 0 });
      return playerIntent({ up: upward, down: !upward });
    }
    case "drop_through": {
      const aligned = Math.abs(self.x - link.fromX) <= tuning.arriveRadiusUnits;
      if (!aligned) return walkToward(self, link.fromX, tuning);
      return playerIntent({ down: true, jump: true });
    }
  }
}
