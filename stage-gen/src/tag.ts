...
/**
 * Transparency changes produced bytes, so it is part of the run/cache identity.
 * Preserve the suffix even when a recipe emits an unusually long base tag.
 */
export function tagForTransparencyMode(
  baseTag: string,
  mode: TransparencyMode,
): string {
  const suffix = `-${mode}`;
  return `${baseTag.slice(0, 128 - suffix.length)}${suffix}`;
}
