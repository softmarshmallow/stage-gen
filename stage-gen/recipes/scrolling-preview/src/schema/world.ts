...
export const WorldLayerSchema = z.object({
  ...
  paint_region: z
    ...
    .min(1)
    .describe(
      "Artwork/foreground bounds in canvas-fraction language: state which Y/X ranges contain visible layer artwork (Y axis 0/5 top to 5/5 bottom) and which ranges remain outside that artwork. Do not name a key colour or removal technique.",
    ),
  description: z
    .string()
    .min(1)
