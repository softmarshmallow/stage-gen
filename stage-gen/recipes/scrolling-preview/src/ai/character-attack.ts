...
// 1 row × 4 columns of attack frames on a removable background, 2400×800.
//
// Inputs:
...
function buildPrompt(): string {
  return (
    ...
    `  - Pixels that are not the character or their weapon/effect are the BACKGROUND FIELD.\n\n` +
...
export async function generateCharacterAttack(args: CharacterAttackArgs) {
  const { prompt, tag, runDir, model, layoutTemplatePath, characterConceptPath, transparencyMode } = args;
