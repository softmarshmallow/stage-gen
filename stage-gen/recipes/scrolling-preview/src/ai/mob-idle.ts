...
// 1 row × 4 columns of idle frames on a removable background, 2400×800.
...
function buildPrompt(mob: WorldSpec["mobs"][number]): string {
  return (
    ...
    `  - The body fits ENTIRELY between the cyan and green rails; pixels outside the silhouette are the BACKGROUND FIELD.\n` +
...
export async function generateAllMobIdles(args: MobIdleArgs) {
  const { prompt, tag, runDir, model, layoutTemplatePath, mobs, transparencyMode } = args;
  return Promise.all(
    mobs.map((mob, i) => {
