...
// strip at 2400×800, then crop the bottom (extra background-field band) down to
...
function railPreamble(): string {
  return (
    ...
    `  IMAGE 1 — LAYOUT TEMPLATE: a 4×1 grid with bright YELLOW grid lines. Its coloured background is layout-only. Each cell has a CYAN horizontal rail (head-top marker) and a single GREEN horizontal rail across the FULL ROW (feet baseline / ground line). The grey humanoid silhouette marks the body's height, width, and centre.\n` +
    ...
    `  • The bottom ${FLOOR_BAND_PX} px of every cell is BACKGROUND FIELD — NOTHING is painted there.\n` +
    `Side view, facing right, throughout the strip.\n` +
    `Every pixel outside the character is the BACKGROUND FIELD described below.\n` +
...
function buildStripPrompt(state: State): string {
  ...
  return (
    ...
    `  - The narrow BACKGROUND FIELD below the green feet rail in every cell must remain empty — no boots, no feet, no shadow, no toes poking through.\n` +
 ...
 * 2400×3440 transparent master sheet via sharp.
 */
export async function generateCharacterCombined(
  ...
  if (!force && (await validCombinedCache(outPath, transparencyMode))) {
  ...
  // Character bodies live at y=80..620 so the trimmed band is transparent.
  ...
  const sourceHashes = await Promise.all(
    stripResults.map(async (result) => ({
      path: result.stripPath,
      sha256: createHash("sha256").update(await readFile(result.stripPath)).digest("hex"),
    })),
  );
  const outputSha256 = createHash("sha256").update(composite).digest("hex");
  const alpha = await alphaFacts(new Uint8Array(composite));
  await writeArtifactWithProvenance(
    outPath,
    { bytes: new Uint8Array(composite), mediaType: "image/png" },
    {
      provider: "local",
      model: "deterministic-alpha-composite",
      seed: null,
      prompt,
      refs: sourceHashes.map((source) => source.path),
      inputs: sourceHashes.map((source) => ({
        ref: source.path,
    ...
    params: {
      ...
      tool: { name: "sharp-alpha-composite", version: sharp.versions.sharp },
    },
  );
...
async function validCombinedCache(
  path: string,
  mode: TransparencyMode,
): Promise<boolean> {
  try {
...
async function alphaFacts(
  ...
  if (info.channels !== 4) throw new Error("character master must be RGBA");
  ...
  if (transparentPixels === 0 || nontransparentPixels === 0) {
    throw new Error("character master must have nontrivial alpha");
  }
