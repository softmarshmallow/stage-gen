...
// Single 4×2 grid of 8 distinct items on a removable background, 2400×800.
...
function buildItemsPrompt(items: WorldSpec["items"]): string {
  ...
  return (
    ...
    `  IMAGE 1 — LAYOUT TEMPLATE: a 4×2 grid of 8 equal cells. Ignore its coloured background and the green band at the bottom of each cell; both become the BACKGROUND FIELD. Treat each cell as a centered slot for ONE item.\n` +
    ...
    `  - Each item is opaque and HARD-edged against the BACKGROUND FIELD — no soft halos, no glow halos painted as soft alpha (any "glow" appearance must be painted as crisp shapes; the runtime applies any glow/bloom procedurally).\n` +
    `  - EVERY pixel that is NOT part of the item is the BACKGROUND FIELD described below.\n` +
...
export async function generateItems(args: ItemsArgs) {
  const { prompt, tag, runDir, model, conceptImagePath, obstacleTemplatePath, items, transparencyMode } = args;
