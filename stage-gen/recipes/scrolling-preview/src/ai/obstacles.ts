...
// 4×2 grid on a removable background field, 2400×800. Uses obstacle_template.png as
...
function buildObstaclePrompt(args: {
  ...
  return (
    ...
    `  IMAGE 1 — LAYOUT TEMPLATE: a 4×2 grid of 8 equal cells. Its coloured background is layout-only. Each cell has a thin GREEN GRASS BAND at the bottom — that band is the ground-contact line where the obstacle rests on the world's ground.\n` +
    ...
    `  - Above the grass band, EVERY pixel that is NOT part of the obstacle is the BACKGROUND FIELD described below.\n` +
    `  - Each obstacle is opaque and HARD-edged against the BACKGROUND FIELD — no soft halos, no glow, no feathered alpha.\n` +
...
export async function generateAllObstacles(args: ObstaclesArgs) {
  const { prompt, tag, runDir, model, conceptImagePath, obstacleTemplatePath, obstacles, transparencyMode } = args;
  return Promise.all(
