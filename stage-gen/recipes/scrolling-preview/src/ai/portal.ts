...
// 2048×1024 with two portals (entry left half / exit right half) on a
// removable background field. Concept attached as the only ref.
...
CONTRACT for both portals:
  ...
  - Outside the portal architecture, EVERY pixel is the BACKGROUND FIELD described below.
  - Each portal is opaque and HARD-edged against the BACKGROUND FIELD — no soft halos, no glow halos painted as soft alpha. The inner aperture glow must stay inside the architecture's frame and end at hard edges.
...
Do NOT render any visible vertical divider line between the halves, no labels, no text, no frame numbers, no grid lines. Just two portals on a shared BACKGROUND FIELD with green grass bands at their feet.
...
export async function generatePortal(args: PortalArgs) {
  const { prompt, tag, runDir, model, conceptImagePath, transparencyMode } = args;
