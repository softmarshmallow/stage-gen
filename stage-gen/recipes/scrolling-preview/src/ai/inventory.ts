...
TWO reference images:
  IMAGE 1 — LAYOUT TEMPLATE: a 1536×1024 canvas with a YELLOW rectangular outline marking the panel's outer edge, and a 4×2 grid of CYAN-outlined square slots inside. Its coloured background is layout-only. The cyan rectangles mark the EXACT positions and sizes of the 8 inventory slots — these must be honoured 1:1 in the output.
...
OUTSIDE THE PANEL (everything beyond the yellow outline) is the BACKGROUND FIELD described below.
...
export async function generateInventory(args: InventoryArgs) {
  const { prompt, tag, runDir, model, conceptImagePath, inventoryTemplatePath, transparencyMode } = args;
