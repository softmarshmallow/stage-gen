...
//
// Single structured-generation call against the configured TEXT_MODEL.
// Reads the concept
...
//
// Retry semantics: the capability owns one five-attempt retry loop around
// transport, decoding, and zod parsing. Schema mismatch and refine() failures
...
import { z } from "zod";
import { aiRequestControl, structuredGenerator } from "./client.ts";
...
export const WORLD_SPEC_SYSTEM_PROMPT = `You are a WORLD-DESIGN AGENT for a procedural 2D side-scrolling platformer.
...
export function buildWorldSpecUserPrompt(
...
REQUIRED: include EXACTLY ONE OPAQUE BACKDROP layer (e.g. sky / nebula / vast distant void). It must have z_index=0, parallax=0, opaque=true. This layer is what shows behind everything else. Every other layer contains bounded foreground artwork; outside its artwork bounds, deeper layers show through.
...
For each layer:
   ...
   - z_index: integer; lowest is drawn first (deepest). Use 0 for the opaque backdrop, then ascending values for each overlay layer painted on top.
   ...
   - paint_region: describe the ARTWORK / FOREGROUND BOUNDS in CANVAS-FRACTION terms: which Y / X ranges contain visible layer artwork and which ranges remain outside that artwork. The Y axis runs 0/5 (top) to 5/5 (bottom). Examples: "artwork occupies Y range 3/5..5/5 (lower 40%); upper 60% remains outside the artwork bounds because the deeper sky covers it", "artwork fills the full canvas" for the opaque sky, "sparse vertical accents at any Y, X anywhere" for foreground vines. Be precise, and do not name a key colour or removal technique.
...
THE RUNTIME HANDLES LOOPING + DEPTH-OF-FIELD: don't paint loop-fade gradients yourself, and don't worry about blur. The runtime crossfades the L/R edges of each non-opaque layer so seams are invisible, and applies a depth-of-field blur derived from the layer's parallax (foreground layers with parallax > 1 get progressively blurred to suggest out-of-focus near-camera depth). Paint each layer SHARP and edge-to-edge as if it were a single isolated panel.`;
}
...
export async function generateWorldSpec(args: WorldSpecArgs): Promise<WorldSpecResult> {
  ...
  const userPromptText = buildWorldSpecUserPrompt(prompt, mobCount, obstacleCount);
  const control = aiRequestControl();
  const generated = await structuredGenerator().generate<WorldSpec>({
    system: WORLD_SPEC_SYSTEM_PROMPT,
    ...
    references: [
      {
        url: `data:image/png;base64,${Buffer.from(conceptBytes).toString("base64")}`,
        provenanceRef: conceptImagePath,
      },
    ],
    schema: {
      name: "scrolling_preview_world_spec",
      description: "World and asset plan for the scrolling preview recipe",
      jsonSchema: z.toJSONSchema(WorldSpecSchema) as any,
      strict: true,
    },
    parse(value) {
      const parsed = WorldSpecSchema.parse(value);
      if (parsed.mobs.length !== mobCount) {
        throw new Error(`mobs length ${parsed.mobs.length} != requested ${mobCount}`);
      }
      if (parsed.obstacles.length !== obstacleCount) {
        throw new Error(
          `obstacles length ${parsed.obstacles.length} != requested ${obstacleCount}`,
        );
      }
      return parsed;
    },
  ...
  const metaPath = generated.provenancePath;
