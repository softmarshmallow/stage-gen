...
def _source(root: Path) -> Path:
    source = root / "library/characters/test-character/profile.toml"
...
def _binding(source: Path) -> dict[str, object]:
    return {
        ...
        "ref": "library/characters/test-character/profile.toml",
...
def test_shared_library_resolver_binds_source_canonical_identity_and_provenance(
    ...
    resolved = resolve_character_profile_binding(_binding(source), character_library_root=tmp_path)
    rerun = resolve_character_profile_binding(_binding(source), character_library_root=tmp_path)
...
def test_shared_library_resolver_rejects_symlinked_root_and_source(
    ...
    with pytest.raises(ValueError, match="root must not traverse a symlink"):
        resolve_character_profile_binding(_binding(source), character_library_root=linked_root)
    ...
    with pytest.raises(ValueError, match="regular non-symlink file"):
        resolve_character_profile_binding(_binding(external), character_library_root=actual)
...
def test_shared_library_resolver_requires_explicit_existing_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="existing directory"):
        resolve_character_profile_binding(
            _binding(source), character_library_root=tmp_path / "missing"
        )
