...
async def test_every_run_reports_the_same_base_shape(tmp_path: Path) -> None:
    ...
    report = run_report(
        completed, run_dir=tmp_path / "run", recipe="pointclick-room", room_id="attic"
    )
    assert set(report) == {
        "ok",
        "run_dir",
    ...
    assert report["node_count"] == len(completed.plan.graph.nodes)
    # A command may override a base key when its meaning differs.
    narrowed = run_report(completed, run_dir=tmp_path / "run", executed_node_count=3, ok=False)
