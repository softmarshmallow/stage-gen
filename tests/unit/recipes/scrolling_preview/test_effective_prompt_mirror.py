...
SHIPPED_GAME = REPOSITORY_ROOT / SHIPPED_REF
...
def _game() -> executor_module._GameContractContext:
    """A real context built from the shipped game, with no run directory involved.
    ...
    resolved = resolve_game_contract_binding(
        {
            ...
            "ref": SHIPPED_REF,
            "source_sha256": hashlib.sha256(SHIPPED_GAME.read_bytes()).hexdigest(),
        },
...
def test_the_mirror_carries_the_art_direction_when_a_game_is_bound() -> None:
    rendered = executor_module._effective_image_prompt(_spec(), None, "chroma", _game())
...
def test_the_art_direction_sits_after_the_prompt_and_before_transparency() -> None:
...
def test_the_clause_is_not_doubled_when_the_prompt_already_carries_it() -> None:
