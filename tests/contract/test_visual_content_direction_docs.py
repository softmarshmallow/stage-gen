...
def test_visual_content_direction_docs_name_the_current_boundary() -> None:
    ...
    legacy_pointer = _read("docs/theme-art-direction.md")
    ...
    assert "[Visual Content Direction](visual-content-direction.md)" in legacy_pointer
...
def test_content_control_spec_tracks_the_v1_model() -> None:
    ...
    assert "no `theme` key preserves the historical graph" in contract
