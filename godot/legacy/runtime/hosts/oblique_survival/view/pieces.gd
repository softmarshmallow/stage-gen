...
const PIECES_SHADER := "res://hosts/oblique_survival/view/shaders/pieces.gdshader"
...
## publishes for every cell (`HostRunDir.check_manifest` refuses one without).
...
## material (`SurvivalCards.set_highlight`); a forage piece is one instance among a
