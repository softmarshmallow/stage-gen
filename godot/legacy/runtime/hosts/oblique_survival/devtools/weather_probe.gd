...
##     -s res://hosts/oblique_survival/devtools/weather_probe.gd -- --run <run dir> --out <png>
##
...
func _init() -> void:
	var args := _args()
	var run_dir := str(args.get("run", _default_run()))
	var out_path := str(args.get("out", "/tmp/weather.png"))
	var pkg: HostRunDir = HostRunDir.open(run_dir)
	if pkg == null:
	...
	var fu: SurvivalFrameUniforms = SurvivalFrameUniforms.from_manifest(manifest, Vector2(SIZE))
	viewport.add_child(_ground(pkg, fu))
	var host := Node3D.new()
	host.name = "Weather"
	viewport.add_child(host)
	var weather := SurvivalWeatherView.new()
	var splashes := SurvivalSplashes.new()
	var strikes := SurvivalStrikes.new()
	var fire := SurvivalFire.new()
	var puffs := SurvivalPuffs.new()
	...
	for frame in range(FRAMES):
		...
		fu.write_frame({
			...
			"u_flash": weather.flash(world) * SurvivalWeatherView.FLASH_GAIN,
		})
		for module: Node3D in _modules:
...
#include "res://hosts/oblique_survival/view/shaders/night.gdshaderinc"
...
"""
	...
	material.set_shader_parameter("level", SurvivalWeatherView._decal_gain(pkg.manifest) / 0.62)
