...
##       -s res://hosts/oblique_survival/devtools/ui_shots.gd -- --run <absolute run dir> --out <directory>
##
...
func _go() -> void:
	_main = load("res://hosts/oblique_survival/main.tscn").instantiate()
	...
	if far_pine != null:
		var toward := Vector2(start.x - float(far_pine["x"]), start.z - float(far_pine["z"])).normalized() * 2.0
		_teleport(float(far_pine["x"]) + toward.x, float(far_pine["z"]) + toward.y)
		_main.press_key("space")
		_main.advance(0.5)
		await _save("ui-noon-refused.png")
		_main.release_key("space")
		_teleport(start.x, start.z)
		_main.advance(0.3)
	...
	SurvivalDropsSystem.spawn_drops(world, [{"item_id": "log", "count": 1}], world.player.x + 1.2, world.player.z, 1.0, 0.0, 0.0)
	...
	SurvivalHelpers.emit(world, {"type": "hurt", "x": world.player.x, "z": world.player.z})
	_main.advance(0.08)
	await _save("ui-hurt-bite.png")
	_main.advance(1.0)
	world.player.hunger = 0.0
	_main.advance(1.25 / SurvivalHurtFlash.THROB_HERTZ)
