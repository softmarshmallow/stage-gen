...
##
##   Godot --path godot/legacy/runtime --resolution 160x100 --position 0,0 \
##       --disable-render-loop --audio-driver Dummy -s res://hosts/oblique_survival/devtools/preview_pieces.gd \
...
func _init() -> void:
	...
	var fu := SurvivalFrameUniforms.from_manifest(manifest, Vector2(SIZE))
	var world := _world_stub(pkg, yaw, String(extra.get("look", "")))

	var pieces := SurvivalPieces.new()
