...
##
##   Godot --path godot/legacy/runtime --rendering-driver metal \
##       --resolution 160x100 --position 0,0 --audio-driver Dummy \
##       -s res://hosts/oblique_survival/devtools/capture_cards.gd -- --run <run dir> --out <png>
##
...
## `SurvivalCards.field`, which takes a Dictionary or an Object), and the ground is a
...
static func _default_run() -> String:
	return ProjectSettings.globalize_path("res://").path_join(DEFAULT_RUN_REF).simplify_path()
...
func _init() -> void:
	...
	var fu := SurvivalFrameUniforms.from_manifest(pkg.manifest, Vector2(SIZE))
	...
	if parts == "gallery":
		...
		var gallery := SurvivalGallery.new()
...
func _reactions(world: Dictionary, cards: SurvivalCards) -> void:
...
## Dictionary world the modules can read through `SurvivalCards.field`.
func _world(pkg: HostRunDir, layout: Dictionary, target: Vector3, yaw: float) -> Dictionary:
...
func _ground(viewport: SubViewport, pkg: HostRunDir) -> void:
...
"""
	...
	material.set_shader_parameter("level", SurvivalDecals.ground_level(pkg.manifest))
