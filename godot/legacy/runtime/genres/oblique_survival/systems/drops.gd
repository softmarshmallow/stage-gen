class_name SurvivalDropsSystem
extends RefCounted
...
static func spawn_drops(
	world: SurvivalWorld, yields: Variant, x: float, z: float,
...
static func pick_up(world: SurvivalWorld, item: Dictionary) -> bool:
	var left := SurvivalInventory.inv_add(world, str(item["item_id"]), 1, item.get("uses", null))
	if left > 0:
