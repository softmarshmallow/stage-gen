...
func _play() -> void:
	var main = load("res://hosts/oblique_survival/main.tscn").instantiate()
	...
	# Take the sampler off the keyboard before the first step: `SurvivalInput`
		...
		"collect":
			...
			if not _world.entities.has(_target):
				...
				_note("log picked up at %.1f s (%d in the pack)" % [
					elapsed, SurvivalInventory.count(_world, "log")])
				_target = null
				_phase = "craft"
				return
...
## space by the camera yaw (`SurvivalPlayerMoveSystem`), so this is that rotation
...
func _timing() -> void:
	...
	for id: String in SurvivalSim.system_micros.keys():
		per_system.append([float(SurvivalSim.system_micros[id]) / float(_timing_frames) / 1000.0, id])
