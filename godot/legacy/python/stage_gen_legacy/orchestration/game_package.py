...
from stage_gen_legacy.recipes.sideview_platformer.validation import (
    ResolvedGamePackage,
    resolve_platformer_member,
)
from stage_gen_legacy.recipes.sideview_runner.validation import resolve_runner_member

MAIN_GAME_SELECTOR_REF = "godot/legacy/inputs/main.toml"
...
class GamePackageSelector(PersistedContractModel):
    ...
    def validate_layout(self) -> GamePackageSelector:
        expected = f"godot/legacy/inputs/{self.game_id}/game.toml"
...
def validate_game_package(
    ...
    *,
    ...
    closure_refs = [
        selector_ref,
        *[f"godot/legacy/inputs/{selector.game_id}/{entry.path}" for entry in resolved.files],
    ]
