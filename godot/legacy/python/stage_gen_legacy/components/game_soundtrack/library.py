...
from stage_gen_legacy.components.game_soundtrack.loader import (
    canonical_game_soundtrack_json,
    load_game_soundtrack_bytes,
)
from stage_gen_legacy.components.game_soundtrack.models import GameSoundtrack, GameSoundtrackBinding

GAME_SOUNDTRACK_LIBRARY_RESOLUTION_VERSION = "game-soundtrack-library-resolution-v1"

_REF_PREFIX = ("godot", "legacy", "inputs")
...
def resolve_game_soundtrack_binding(
    ...
    *,
    ...
    """Resolve ``godot/legacy/inputs/<game_id>/soundtrack.toml`` without symlinks."""

    binding = GameSoundtrackBinding.model_validate(value)
    parts = PurePosixPath(binding.ref).parts
    if len(parts) != 5 or parts[:3] != _REF_PREFIX or parts[4] != _REF_FILENAME:
        raise ValueError(
            "game soundtrack ref must equal godot/legacy/inputs/<game_id>/soundtrack.toml"
        )
