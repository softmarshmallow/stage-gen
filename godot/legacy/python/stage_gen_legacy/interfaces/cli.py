...
from stage_gen_legacy.application import resolve_genre
from stage_gen_legacy.components.case import ResolvedCase, read_case_catalog, resolve_case
from stage_gen_legacy.components.game_soundtrack import (
    ResolvedGameSoundtrack,
    resolve_game_soundtrack_binding,
)
from stage_gen_legacy.model_policy import (
    build_repository_model_policy_projection,
    load_active_model_policy_snapshot,
)
from stage_gen_legacy.orchestration.case_binding import BoundCase, bind_case
from stage_gen_legacy.orchestration.case_bundle import publish_case
from stage_gen_legacy.orchestration.game_package import resolve_prepared_package
from stage_gen_legacy.recipes.dialogue_scene.review import transition_dialogue_review
from stage_gen_legacy.recipes.dialogue_scene.scene_executor import DialogueSceneExecutor
from stage_gen_legacy.recipes.dialogue_scene.scene_view import build_dialogue_scene_view
...
from stage_gen_legacy.recipes.oblique_survival.survival_view import build_oblique_survival_view
...
from stage_gen_legacy.recipes.sideview_platformer.execution_view import build_execution_view
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for action in ("validate", "digest"):
        ...
        soundtrack_action_parser.add_argument(
            "--game-library-root",
            required=True,
            help="workspace root containing godot/legacy/inputs",
        )
...
def _resolve_cli_game_soundtrack(
    *, input_path: Path, game_library_root: Path
    ...
    if (
        len(parts) != 5
        or parts[:3] != ("godot", "legacy", "inputs")
        or parts[4] != "soundtrack.toml"
    ):
        raise ValueError(
