...
`main.toml` is the default selector for the legacy runtime only. It selects one
...
- Changes to `main.toml` or its selected closure keep the member path list exact. Preserve supported
  legacy schemas with their readers. These contracts are local to the legacy demos; new asset pipelines
  and independent games do not have to adopt them or convert existing TOML to a newer universal schema.
...
- Validate the authored closure from the repository root with
  `uv run python godot/legacy/tools/validate_game_package.py --root .`. It parses every TOML, confines every
