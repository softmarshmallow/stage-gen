...
if [ "$SKIP_TESTS" -eq 0 ]; then
  ...
  #
  # 326 — frame 327 is the one replacement creature the route spawns after a
  # kill, and the port puts it in a different column. Not an unported system;
  # `tests/test_platformer.gd` carries the measurement that would close it.
  ...
  python3 "$PROJECT/tools/frames_prefix.py" \
      ...
      "$PLATFORMER_REPLAY/01-village-600.web-frames.txt" --at-least 326
  python3 "$PROJECT/tools/runner_parity_diff.py" \
