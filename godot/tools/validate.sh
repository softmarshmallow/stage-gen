...
if [ "$SKIP_TESTS" -eq 0 ]; then
  ...
  #
  # 196 — frame 197 is the first contact between a creature and the body.
  # `tests/test_platformer.gd` carries the same number and reason.
  ...
  python3 "$PROJECT/tools/frames_prefix.py" \
      ...
      "$PLATFORMER_REPLAY/01-village-600.web-frames.txt" --at-least 196
  python3 "$PROJECT/tools/runner_parity_diff.py" \
