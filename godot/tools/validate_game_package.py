#!/usr/bin/env python3
"""Validate an explicitly selected maintained game input package."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=("Validate one exact-current game source closure without rewriting it.")
    )
    parser.add_argument("--input", type=Path, required=True, help="game input directory or ZIP")
    parser.add_argument(
        "--repository-root", type=Path, help="explicit checkout for optional Git checks"
    )
    parser.add_argument(
        "--require-tracked",
        action="store_true",
