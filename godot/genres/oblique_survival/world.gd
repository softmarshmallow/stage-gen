...
var player: SurvivalPlayerState = null
...
## is `{item, count, uses}` or null; `SurvivalInventory.equip` and `unequip` move
...
## shot (cleared by `SurvivalSim` after every step): everything else.
...
## Hoisting it onto the world keeps `SurvivalSim.tick` a pure function of the world.
...
## `SurvivalMasks` to adopt instead of rebuilding, which is how a reset keeps them).
static func create(pkg: HostRunDir, seed_value: int, opts: Dictionary = {}) -> SurvivalWorld:
	...
	var player := SurvivalPlayerState.new()
...
static func reset(old: SurvivalWorld) -> SurvivalWorld:
...
func friction_at(x: float, z: float) -> float:
	if masks == null:
		return SurvivalMasks.DEFAULT_FRICTION
...
static func _create_season(manifest: Dictionary, force: String) -> Dictionary:
	...
	return {
		...
		"index": 0,
		"day_in_season": 1,
		"spec": SurvivalHelpers.NO_SEASON,
		"turns": 0,
	}
