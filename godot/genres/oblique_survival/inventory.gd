...
## viewer did not have (`SurvivalWorld.equipment`): a tool is worn in the `hand`, a
...
static func item_spec(world: SurvivalWorld, item_id: String) -> Variant:
...
static func item_name(world: SurvivalWorld, item_id: String) -> String:
...
## until it is worn.
static func slot_capacity(world: SurvivalWorld) -> int:
...
static func insulation(world: SurvivalWorld) -> float:
...
static func equip(world: SurvivalWorld, index: int) -> bool:
	...
	SurvivalHelpers.say(world, "%s: %s." % [kind, item_name(world, item_id)])
	return true
...
## it came off.
static func unequip(world: SurvivalWorld, kind: String) -> bool:
	var worn: Variant = world.equipment.get(kind, null)
	if worn == null:
		return false
	...
	if kind == "back":
		...
		for i in world.slots.size():
			if world.slots[i] != null and i >= after:
				SurvivalHelpers.say(world, "Empty the pack's own slots first.")
				return false
	...
	SurvivalHelpers.say(world, "%s off." % item_name(world, str(entry["item"])))
	return true


## How many of an item the pack holds (the viewer's invCount).
static func count(world: SurvivalWorld, item_id: String) -> int:
	var total := 0
...
static func has(world: SurvivalWorld, item_id: String, n: int = 1) -> bool:
	return count(world, item_id) >= n


## Add up to ``amount``; returns how many did not fit. A tool keeps its wear.
static func inv_add(world: SurvivalWorld, item_id: String, amount: int, uses: Variant = null) -> int:
	var spec: Variant = item_spec(world, item_id)
	var stack_max := 1
	if spec != null:
...
## came out.
static func inv_remove(world: SurvivalWorld, item_id: String, amount: int) -> int:
	var left := amount
...
static func inv_find_tool(world: SurvivalWorld, verb: String) -> int:
...
static func wear_tool(world: SurvivalWorld, index: int) -> void:
	...
	if int(entry["uses"]) <= 0:
		if index == HAND_SLOT:
			...
			world.slots[index] = null
		SurvivalHelpers.say(world, "The %s breaks." % item_name(world, str(entry["item"])))
