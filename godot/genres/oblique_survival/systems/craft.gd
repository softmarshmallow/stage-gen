class_name SurvivalCraftSystem
extends RefCounted
...
static func update(world: SurvivalWorld, _dt: float) -> void:
	...
	if bool(world.input["craft_toggle"]):
		if recipes.is_empty():
			SurvivalHelpers.say(world, "This run has no crafting table.")
			return
...
static func _recipes(world: SurvivalWorld) -> Array:
...
static func station_near(world: SurvivalWorld, station_id: String) -> Variant:
	var crafting: Variant = world.manifest.get("crafting", null)
	if crafting == null:
		return null
...
## is in reach.
static func recipe_status(world: SurvivalWorld, recipe: Dictionary) -> Dictionary:
	var short: Array = []
	var ingredients: Dictionary = recipe["ingredients"]
	for item_id in ingredients:
		var wanted := int(ingredients[item_id])
		var held := SurvivalInventory.count(world, str(item_id))
		if held < wanted:
...
## the player's own.
static func place_verdict(world: SurvivalWorld, x: float, z: float, radius: float) -> String:
	var player: SurvivalPlayerState = world.player
	var dx := x - player.x
	var dz := z - player.z
	var distance := sqrt(dx * dx + dz * dz)
	if distance > SurvivalTargeting.approach_of(world):
		return WHY_FAR
	if not bool(world.is_land.call(x, z)):
		return WHY_WATER
	if distance < radius + player.radius + 0.2 or SurvivalTargeting.footprint_blocked(world, x, z, radius):
		return WHY_ROOM
	return ""
...
static func _update_placing(world: SurvivalWorld) -> void:
	...
	if built == null:
		SurvivalHelpers.say(world, WHY_ROOM)
		return
	var ingredients: Dictionary = recipe["ingredients"]
	for item_id in ingredients:
		SurvivalInventory.inv_remove(world, str(item_id), int(ingredients[item_id]))
	world.placing = null
	SurvivalHelpers.say(world, "%s built." % str(placing["prop_id"]).replace("_", " ").capitalize())
	SurvivalHelpers.emit(world, {"type": "craft", "recipe": str(recipe["recipe_id"])})


## Let the build go: nothing was spent, so there is nothing to give back.
static func cancel_placing(world: SurvivalWorld) -> void:
	if world.placing == null:
		return
	var name := str((world.placing as Dictionary)["prop_id"]).replace("_", " ")
	world.placing = null
	if not world.dead:
		SurvivalHelpers.say(world, "The %s is not built; the makings are kept." % name)
...
static func place_prop(world: SurvivalWorld, prop_id: String, state: String, x: float, z: float) -> Variant:
...
static func place_start(world: SurvivalWorld, radius: float) -> Dictionary:
	var player: SurvivalPlayerState = world.player
	var forward := SurvivalTargeting.facing_direction(world)
...
static func craft_recipe(world: SurvivalWorld, recipe: Dictionary) -> bool:
	var status := recipe_status(world, recipe)
	var short: Array = status["short"]
	if not short.is_empty():
		var parts: Array = []
		for pair in short:
			parts.append("%d %s" % [int(pair[1]), SurvivalInventory.item_name(world, str(pair[0]))])
		SurvivalHelpers.say(world, "Need %s." % ", ".join(parts))
		return false
	if not bool(status["station_ok"]):
		SurvivalHelpers.say(world, "Needs a %s within reach." % str(recipe["station"]).replace("_", " "))
		return false
	...
	if prop_id != null and str(prop_id) != "":
		...
		SurvivalHelpers.say(world, "Click where the %s goes; right-click to keep the makings." % str(prop_id).replace("_", " "))
		SurvivalHelpers.emit(world, {"type": "place", "recipe": str(recipe["recipe_id"])})
		return true
	for item_id in ingredients:
		SurvivalInventory.inv_remove(world, str(item_id), int(ingredients[item_id]))
	...
	var left := SurvivalInventory.inv_add(world, made, count)
	var worn := ""
	if left == 0:
		worn = wear_if_free(world, made)
	if left > 0:
		var forward := SurvivalTargeting.facing_direction(world)
		SurvivalDropsSystem.spawn_drops(
			world, [{"item_id": made, "count": left}], world.player.x, world.player.z,
			float(forward["x"]), float(forward["z"]), 0.3
		)
		SurvivalHelpers.say(world, "Made %s; the pack is full, so it falls at your feet." % SurvivalInventory.item_name(world, made))
	elif worn != "":
		SurvivalHelpers.say(world, "Made %s, %s." % [SurvivalInventory.item_name(world, made), str(WORN_WORDS[worn])])
	elif count > 1:
		SurvivalHelpers.say(world, "Made %s (%d)." % [SurvivalInventory.item_name(world, made), count])
	else:
		SurvivalHelpers.say(world, "Made %s." % SurvivalInventory.item_name(world, made))
	SurvivalHelpers.emit(world, {"type": "craft", "recipe": str(recipe["recipe_id"])})
	return true
...
static func wear_if_free(world: SurvivalWorld, item_id: String) -> String:
	var kind := SurvivalInventory.equip_kind(world, item_id)
	...
	for index in range(world.slots.size() - 1, -1, -1):
		...
		if entry is Dictionary and str((entry as Dictionary).get("item", "")) == item_id:
			return kind if SurvivalInventory.equip(world, index) else ""
	return ""
