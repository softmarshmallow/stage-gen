class_name SurvivalMobAiSystem
extends RefCounted
...
static func update(world: SurvivalWorld, dt: float) -> void:
	...
	for mob in world.entities:
		...
		if str(mob["state"]) == "attack":
			...
			if float(mob["elapsed"]) < SurvivalTargeting.state_duration(states.get("attack", null)):
				continue
			mob["state"] = "idle"
			mob["elapsed"] = 0.0
			...
			if reach_distance <= attack_range + player.radius + float(mob["radius"]) \
				...
				if states.has("hurt"):
					player.state = "hurt"
				player.elapsed = 0.0
				SurvivalHelpers.emit(world, {"type": "hurt", "x": player.x, "z": player.z})
			continue
		...
		if distance <= attack_range + player.radius + float(mob["radius"]) and float(mob["cooldown"]) <= 0.0:
			...
			mob["vx"] = 0.0
			mob["vz"] = 0.0
			mob["facing"] = SurvivalTargeting.facing_for(dx, dz, world.camera_yaw, str(mob["facing"]))
		elif distance < aggro:
		...
		if bool(world.is_land.call(next_x, next_z)):
			...
			mob["vx"] = -float(mob["vx"])
			mob["vz"] = -float(mob["vz"])
		mob["facing"] = SurvivalTargeting.facing_for(float(mob["vx"]), float(mob["vz"]), world.camera_yaw, str(mob["facing"]))
...
static func _states(world: SurvivalWorld, actor_id: String) -> Dictionary:
