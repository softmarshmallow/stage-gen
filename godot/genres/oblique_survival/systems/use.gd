class_name SurvivalUseSystem
extends RefCounted

## Reads `input`, `selection`, `inventory`; writes `inventory_use`, `torch`,
## `warm`, `vitals_delta`. viewer/index.html 788-845 and 1305-1313.


static func declaration() -> KernelSystem:
	return KernelSystem.of({
		"id": "survival/use",
		"contract_version": "survival-use-system-v1",
		"reads": ["input", "selection", "inventory"],
		"writes": ["inventory_use", "torch", "warm", "vitals_delta"],
	})

static func update(world: SurvivalWorld, _dt: float) -> void:
	if bool(world.input["use"]):
		use_selected(world)
	if bool(world.input.get("equip", false)):
		SurvivalInventory.equip(world, world.selected)
	var off: Variant = world.input.get("unequip", null)
	if off != null and SurvivalInventory.EQUIPMENT_KINDS.has(str(off)):
		SurvivalInventory.unequip(world, str(off))
	if bool(world.input["drop"]):
		drop_selected(world)


## X: eat or apply the selected item, light it as a torch, or wear it (a
## tool, a cloak, a pack) — not the viewer's, where a worn thing worked from
## the pack and using it said so.
static func use_selected(world: SurvivalWorld) -> void:
	var slot: Variant = null
	if world.selected >= 0 and world.selected < world.slots.size():
		slot = world.slots[world.selected]
	if slot == null:
		SurvivalHelpers.say(world, "Nothing selected.")
		return
	var entry := slot as Dictionary
	if SurvivalInventory.equip_kind(world, str(entry["item"])) != "":
		SurvivalInventory.equip(world, world.selected)
		return
	var spec: Variant = SurvivalInventory.item_spec(world, str(entry["item"]))
	var use: Variant = (spec as Dictionary).get("use", null) if spec != null else null
	var display := SurvivalInventory.item_name(world, str(entry["item"]))
	if use == null:
		SurvivalHelpers.say(world, "The %s is not for using here; Z drops it." % display)
		return
	var block := use as Dictionary
	var kind := str(block.get("kind", ""))
	if kind == "consume":
		var player: SurvivalPlayerState = world.player
		var rules: Dictionary = world.manifest["gameplay"]
		var hunger_max := _limit(rules, "hunger")
		var health_max := _limit(rules, "health")
		var warmth_max := _limit(rules, "warmth")
		player.hunger = maxf(0.0, minf(hunger_max, player.hunger + float(block.get("hunger", 0.0))))
		player.health = maxf(0.0, minf(health_max, player.health + float(block.get("health", 0.0))))
		player.warmth = maxf(0.0, minf(warmth_max, player.warmth + float(block.get("warmth", 0.0))))
		_consume_one(world, entry)
		SurvivalHelpers.emit(world, {"type": "eat", "item": str(entry["item"])})
		if float(block.get("health", 0.0)) > 0.0 and float(block.get("hunger", 0.0)) == 0.0:
			SurvivalHelpers.say(world, "The %s helps." % display)
		else:
			SurvivalHelpers.say(world, "Ate %s." % display)
	elif kind == "light":
		if world.torch["remaining"] > 0.0:
			SurvivalHelpers.say(world, "A torch is already lit.")
			return
		var burn := float(block.get("burn_seconds", 0.0))
		var radius := float(block.get("radius_meters", 0.0))
		world.torch = {
			"remaining": burn if burn != 0.0 else 60.0,
			"radius": radius if radius != 0.0 else 3.0,
		}
		_consume_one(world, entry)
		SurvivalHelpers.emit(world, {"type": "puff", "kind": "sparkle", "x": world.player.x, "z": world.player.z})
		SurvivalHelpers.say(world, "The %s catches." % display)
	elif kind == "warm":
		if world.warm["remaining"] > 0.0:
			SurvivalHelpers.say(world, "A stone is already warm.")
			return
		var heat := float(block.get("heat_seconds", 0.0))
		world.warm = {"remaining": heat if heat != 0.0 else 120.0}
		_consume_one(world, entry)
		SurvivalHelpers.emit(world, {"type": "puff", "kind": "sparkle", "x": world.player.x, "z": world.player.z})
		SurvivalHelpers.say(world, "The %s's heat spreads." % display)


## Z: one of the selected item, dropped at the feet. (A pack in a slot is not
## the one on the back, so it drops like anything else.)
static func drop_selected(world: SurvivalWorld) -> void:
	var slot: Variant = null
	if world.selected >= 0 and world.selected < world.slots.size():
		slot = world.slots[world.selected]
	if slot == null:
		return
	var entry := slot as Dictionary
	var uses: Variant = entry["uses"]
	_consume_one(world, entry)
	var forward := SurvivalTargeting.facing_direction(world)
	SurvivalDropsSystem.spawn_drops(
		world, [{"item_id": str(entry["item"]), "count": 1}],
		world.player.x, world.player.z, float(forward["x"]), float(forward["z"]), 0.3, uses
	)
	# The same sound as a pickup; `out` tells the HUD the thing left the pack
	# rather than arrived, so nothing flies into a slot.
	SurvivalHelpers.emit(world, {
		"type": "pickup", "item": str(entry["item"]), "x": world.player.x, "z": world.player.z, "out": true,
	})


static func _consume_one(world: SurvivalWorld, entry: Dictionary) -> void:
	entry["count"] = int(entry["count"]) - 1
	if int(entry["count"]) <= 0:
		world.slots[world.selected] = null


static func _limit(rules: Dictionary, key: String) -> float:
	var block: Variant = rules.get(key, null)
	if block == null:
		return 100.0
	var authored := float((block as Dictionary).get("max", 0.0))
	return authored if authored != 0.0 else 100.0
