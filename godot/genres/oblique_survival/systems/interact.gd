class_name SurvivalInteractSystem
extends RefCounted
...
static func update(world: SurvivalWorld, dt: float) -> void:
	...
	if world.dead:
		_aim(world, null)
		return
	var reach := SurvivalTargeting.reach_of(world)
	if player.approach != null:
		...
		if not bool((target as Dictionary).get("ready", true)):
			...
			return
		SurvivalTargeting.start_interaction(world, target as Dictionary)
		return
	...
	if clicked is Dictionary:
		...
		if SurvivalTargeting.index_of(world.entities, clicked) >= 0:
			chosen_by_click = SurvivalTargeting.target_for(world, clicked as Dictionary)
		player.goto = null
		if chosen_by_click == null:
			_aim(world, SurvivalTargeting.interactable_at(world))
			SurvivalHelpers.say(world, "Nothing to be done with that.")
			return
		...
		if float((chosen_by_click as Dictionary)["edge"]) > reach \
			...
			return
		SurvivalTargeting.start_interaction(world, chosen_by_click as Dictionary)
		return
	if SurvivalTargeting.yield_pending(world):
...
static func _aim(world: SurvivalWorld, focus: Variant) -> void:
	world.focus = focus
...
static func _refuse(world: SurvivalWorld, target: Dictionary) -> void:
	var entity: Dictionary = target["entity"]
	var player: SurvivalPlayerState = world.player
	player.facing = SurvivalTargeting.facing_for(
		float(entity["x"]) - player.x, float(entity["z"]) - player.z, world.camera_yaw, player.facing
	)
	var text := str(target["disabled"])
	SurvivalHelpers.say(world, text.substr(0, 1).to_upper() + text.substr(1) + ".")


static func _advance_busy(world: SurvivalWorld, player: SurvivalPlayerState, states: Dictionary, dt: float) -> void:
	...
	if entity is Dictionary and SurvivalTargeting.index_of(world.entities, entity) >= 0:
		_aim(world, SurvivalTargeting.target_for(world, entity as Dictionary))
	busy["elapsed"] = float(busy["elapsed"]) + dt
	if float(busy["elapsed"]) < SurvivalTargeting.state_duration(states.get(str(busy["state"]), null)):
		return
	...
	elif entity != null and SurvivalTargeting.index_of(world.entities, entity) >= 0:
	...
	_aim(world, null if SurvivalTargeting.yield_pending(world) else SurvivalTargeting.interactable_at(world))


static func _finish_take(world: SurvivalWorld, player: SurvivalPlayerState, entity: Dictionary) -> void:
	...
	if str(entity.get("kind", "")) == "forage":
		...
		var left := SurvivalInventory.inv_add(world, str(entity["item_id"]), wanted)
		if left == wanted:
			entity["taken"] = false
			SurvivalHelpers.say(world, "Hands full.")
		else:
			entity["picked"] = true
		...
		return
	var index := SurvivalTargeting.index_of(world.entities, entity)
	if index >= 0:
		if SurvivalDropsSystem.pick_up(world, entity):
...
static func _land_blow(
	world: SurvivalWorld, player: SurvivalPlayerState, entity: Dictionary,
	...
	if hits >= busy_hits:
		...
		if str(interaction.get("yield_to", "")) == "hand":
			...
			var toppling := -1.0 if SurvivalTargeting.screen_right_component(away_x, away_z, world.camera_yaw) < 0.0 else 1.0
...
static func _take_in_hand(
	world: SurvivalWorld, _player: SurvivalPlayerState, entity: Dictionary, yields: Variant, dir_x: float, dir_z: float
) -> void:
	if yields == null:
		return
	var fallen: Array = []
	for produced in (yields as Array):
		...
		if wanted == 0:
			wanted = 1
		var left := SurvivalInventory.inv_add(world, str(block["item_id"]), wanted)
	...
	if not fallen.is_empty():
		SurvivalHelpers.say(world, "Hands full.")
		SurvivalDropsSystem.spawn_drops(world, fallen, float(entity["x"]), float(entity["z"]), dir_x, dir_z, 1.0)
