...
static func prop_spec(world: SurvivalWorld, entity: Dictionary) -> Variant:
...
static func player_states(world: SurvivalWorld) -> Dictionary:
...
static func facing_direction(world: SurvivalWorld) -> Dictionary:
...
static func reach_of(world: SurvivalWorld) -> float:
...
static func approach_of(world: SurvivalWorld) -> float:
...
static func push_out_of_footprints(world: SurvivalWorld, player: SurvivalPlayerState) -> void:
...
static func footprint_blocked(world: SurvivalWorld, x: float, z: float, radius: float) -> bool:
	for entity in world.entities:
...
static func target_for(world: SurvivalWorld, entity: Dictionary) -> Variant:
...
static func _prop_target(world: SurvivalWorld, entity: Dictionary, spec: Variant, block: Dictionary, edge: float) -> Dictionary:
	...
	if tool_spec != null:
		...
		tool_slot = SurvivalInventory.inv_find_tool(world, verb)
		if tool_slot != -1:
...
## an axe is the focus, with its refusal on it; `SurvivalInteractSystem` makes it the
...
static func interactable_at(world: SurvivalWorld) -> Variant:
...
static func start_interaction(world: SurvivalWorld, target: Dictionary) -> void:
	var player: SurvivalPlayerState = world.player
	...
	if target["disabled"] != null:
		# Never a target (`SurvivalInteractSystem._aim`): the refusal is said in its
...
static func _centre_distance(player: SurvivalPlayerState, entity: Dictionary) -> float:
	var dx: float = player.x - float(entity["x"])
	var dz: float = player.z - float(entity["z"])
...
static func yield_pending(world: SurvivalWorld) -> bool:
	var notice := maxf(reach_of(world), approach_of(world))
	var player: SurvivalPlayerState = world.player
