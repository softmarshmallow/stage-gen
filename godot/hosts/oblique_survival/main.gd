...
const MODULE_FILES := [
	"res://hosts/oblique_survival/view/ground.gd",
	"res://hosts/oblique_survival/view/water.gd",
	"res://hosts/oblique_survival/view/decals.gd",
	"res://hosts/oblique_survival/view/shadows.gd",
	"res://hosts/oblique_survival/view/cards.gd",
	"res://hosts/oblique_survival/view/pieces.gd",
	"res://hosts/oblique_survival/view/fire.gd",
	"res://hosts/oblique_survival/view/puffs.gd",
	"res://hosts/oblique_survival/view/splashes.gd",
	"res://hosts/oblique_survival/view/strikes.gd",
	"res://hosts/oblique_survival/view/weather_view.gd",
	"res://hosts/oblique_survival/view/gallery.gd",
	"res://hosts/oblique_survival/view/hurt_flash.gd",
	"res://hosts/oblique_survival/view/warmth_veil.gd",
	"res://hosts/oblique_survival/hud/hud.gd",
	"res://hosts/oblique_survival/hud/craft_panel.gd",
	"res://hosts/oblique_survival/hud/death_screen.gd",
	"res://hosts/oblique_survival/hud/world_map.gd",
	"res://hosts/oblique_survival/hud/pause_menu.gd",
	"res://hosts/oblique_survival/audio/music.gd",
	"res://hosts/oblique_survival/audio/sfx.gd",
]
...
## Turning it on also profiles the simulation's fifteen systems (`SurvivalSim.profile`)
...
func _ready() -> void:
	...
	if package == null:
		...
		return
	_weather_script = _load_script("res://genres/oblique_survival/systems/weather.gd")
	_input_script = _load_script("res://hosts/oblique_survival/input.gd")
...
func _boot(next_world: SurvivalWorld) -> void:
	world = next_world
	generation += 1
	frame_uniforms = SurvivalFrameUniforms.from_manifest(package.manifest, resolution())
	...
	environment_node = load("res://hosts/oblique_survival/view/environment.gd").new()
	environment_node.name = "Environment"
	add_child(environment_node)
	environment_node.setup(package, world, frame_uniforms)

	rig = SurvivalCameraRig.new()
	rig.name = "SurvivalCameraRig"
	add_child(rig)
	...
	vignette = load("res://hosts/oblique_survival/view/vignette.gd").new()
...
## the weather mode and the forced season (`SurvivalWorld.reset`). The camera, the
## music and every card start over with it.
func reset() -> void:
	if world == null:
		return
	var next_world := SurvivalWorld.reset(world)
	_teardown()
	_boot(next_world)
	paused = false
	_pointer_down = false
...
func set_clock(phase: float) -> void:
	...
	world.night = SurvivalHelpers.night_factor(world.day_phase)
...
func status() -> String:
	...
	return "%s | season %s | music %s | day %d phase %.3f night %.3f | player %.2f,%.2f" % [
		SurvivalHelpers.describe_weather(world),
		SurvivalHelpers.describe_season(world),
		music_line,
		world.day,
		world.day_phase,
		world.night,
...
func give(item_id: String, count: int = 1) -> int:
	if world == null:
		return count
	return SurvivalInventory.inv_add(world, item_id, count)
...
func resolution() -> Vector2:
	...
	if viewport == null:
		return Vector2(SurvivalCameraRig.VERDICT_SIZE)
...
func _refresh_hover() -> void:
	...
	# A forage piece has no card; the sheet lifts it by instance (`SurvivalPieces`).
...
func _follow_hover() -> void:
	...
	var pieces_node = modules.get("pieces")
	var entity: Variant = _hover.get("entity")
	if entity is Dictionary and SurvivalTargeting.index_of(world.entities, entity) < 0:
		_hover = {}
		entity = null
		if cards_node != null and cards_node.has_method("set_highlight"):
		...
		_hover["target"] = SurvivalTargeting.target_for(world, entity as Dictionary)
...
## would do to the entity (`SurvivalTargeting.target_for`), null when it offers
...
func _pick(screen: Vector2) -> Dictionary:
	...
	if cards_node != null and cards_node.has_method("pick_entity"):
		...
		if entity is Dictionary:
			found["entity"] = entity
			found["target"] = SurvivalTargeting.target_for(world, entity as Dictionary)
...
## simulation step; `SurvivalSim` clears it after every step.
