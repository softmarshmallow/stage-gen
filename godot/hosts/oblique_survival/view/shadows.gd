...
##
## This module reads only the manifest, the world and `SurvivalCards`'s static layout
...
const SHADOW_SHADER := "res://hosts/oblique_survival/view/shaders/shadow.gdshader"
...
func setup(pkg, world, _fu) -> void:
	...
	_look = String(SurvivalCards.field(world, "look", ""))
...
func update(world, _delta: float, _cam: Dictionary) -> void:
	var look := String(SurvivalCards.field(world, "look", ""))
	if look != _look:
		set_look(look)
	sync(world)
...
## `SurvivalCards.field`, the kind is compared as a Variant rather than built into a
...
func sync(world) -> void:
	...
	for entity: Variant in SurvivalCards.field(world, "entities", []):
		...
		if kind == "prop":
			...
			if actors < actors_cap:
				...
				actors += 1
	var player: Variant = SurvivalCards.field(world, "player")
	var player_id := String(SurvivalCards.field(world, "player_id", ""))
	if player != null and player_id != "" and actors < actors_cap:
		_lay(
			actors_mesh, _laid_actors, actors,
			float(SurvivalCards.field(player, "x", 0.0)), float(SurvivalCards.field(player, "z", 0.0)),
...
func _prop_half_width(prop_id: Variant, state: Variant) -> float:
	...
	if prop is Dictionary:
		var spec := SurvivalCards.state_spec(prop, String(state), _look)
		if not spec.is_empty():
			var authored := float((prop as Dictionary).get("shadow_width_meters", 0.0))
			half = 0.5 * (authored if authored != 0.0 else float(SurvivalCards.card_layout(spec)["width"]) * 0.5)
...
func _item_half_width(item_id: String) -> float:
	...
	if spec is Dictionary:
		...
		half = float(SurvivalCards.card_layout(spec)["width"]) * 0.9 * 0.5
