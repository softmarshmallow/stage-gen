class_name SurvivalWeatherView
extends Node3D
...
##   conditional decals itself, wire this and SurvivalWeatherView drops its own pool and
##   forwards `update_wet` instead. Exactly one of the two draws the puddles.
## - `flash(world) * SurvivalWeatherView.FLASH_GAIN` is what Main writes into
##   SurvivalFrameUniforms as `u_flash`.

const RAIN_SHADER := "res://hosts/oblique_survival/view/shaders/rain.gdshader"
const WET_SHADER := "res://hosts/oblique_survival/view/shaders/wet_decal.gdshader"
