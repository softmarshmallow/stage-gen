...
func setup(pkg, world, fu) -> void:
	...
	orient(float(SurvivalCards.field(world, "camera_yaw", 0.0)))
...
func update(world, _delta: float, cam: Dictionary) -> void:
	...
	var weather: Variant = SurvivalCards.field(world, "weather", {})
	var snow := float(SurvivalCards.field(weather, "snow", 0.0))
	var wet := float(SurvivalCards.field(weather, "wet", 0.0))
	set_snow_cover(snow)
...
func _group(spec: Dictionary, entries: Array, conditional: bool, gain: float) -> Dictionary:
	...
	material.set_shader_parameter("u_alpha_cutoff", SurvivalCards.ALPHA_CUTOFF)
