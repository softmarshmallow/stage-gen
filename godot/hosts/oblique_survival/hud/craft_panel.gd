...
##
## The sim owns the table (`SurvivalCraftSystem`): C or Escape toggles it, W/S and Enter
...
func _build() -> void:
	...
	var title := SurvivalUiKit.label("Craft", SurvivalUiKit.TITLE, SurvivalUiKit.ACCENT)
	title.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	header.add_child(title)
	_hint = SurvivalUiKit.rich(SurvivalUiKit.SMALL)
	_hint.text = "[color=#e8e4dc99]click a row · double-click or %s makes · %s / %s choose · %s closes[/color]" % [
		SurvivalUiKit.kbd("Enter"), SurvivalUiKit.kbd("W"), SurvivalUiKit.kbd("S"), SurvivalUiKit.kbd("C")]
	_hint.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_hint.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	header.add_child(_hint)
	var close := SurvivalUiKit.button("×", SurvivalUiKit.TITLE)
	...
	var close_button := SurvivalUiKit.button("Close")
	close_button.pressed.connect(_on_close)
	footer.add_child(close_button)
	_craft_button = SurvivalUiKit.button("Craft")
...
func _row(world, recipe: Dictionary, index: int, selected: bool, state: Dictionary) -> Control:
	...
	if item_id != null:
		...
		name_text = SurvivalUiKit.item_name(manifest, product_id) + ((" ×%d" % many) if many > 1 else "")
	else:
		name_text = "%s · build" % SurvivalUiKit.prop_name(manifest, product_id)
	...
	var name_label := SurvivalUiKit.label(name_text, SurvivalUiKit.FONT_SIZE, SurvivalUiKit.ACCENT if selected else SurvivalUiKit.TEXT)
	...
	for id: String in ingredients.keys():
		...
		var have := SurvivalUiKit.inv_count(world.slots, id)
		...
		var short := have < want
		var text := SurvivalUiKit.label("%s %d/%d" % [SurvivalUiKit.item_name(manifest, id), have, want],
			SurvivalUiKit.SMALL, SurvivalUiKit.SHORT if short else SurvivalUiKit.MUTED)
	...
	var station := SurvivalUiKit.label(station_text, SurvivalUiKit.SMALL,
		SurvivalUiKit.MUTED if bool(state["station_ok"]) else SurvivalUiKit.SHORT)
...
class CraftRow:
	...
	func _draw() -> void:
		...
		if selected:
			box.bg_color = SurvivalUiKit.ACCENT_DIM
			box.border_color = SurvivalUiKit.ACCENT
