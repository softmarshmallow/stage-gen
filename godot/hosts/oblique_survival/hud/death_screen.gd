class_name SurvivalDeathScreen
extends CanvasLayer
...
func setup(pkg, world, _fu) -> void:
	...
	_headline = SurvivalUiKit.label("", HEADLINE, SurvivalUiKit.ACCENT)
	_headline.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	box.add_child(_headline)
	_line = SurvivalUiKit.label("", SurvivalUiKit.FONT_SIZE, SurvivalUiKit.TEXT)
	_line.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_line.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_line.custom_minimum_size = Vector2(PANEL_WIDTH - 44.0, 0.0)
	box.add_child(_line)
	_run_line = SurvivalUiKit.label("", SurvivalUiKit.SMALL, SurvivalUiKit.MUTED)
	_run_line.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	box.add_child(_run_line)
	box.add_child(SurvivalUiKit.spacer(10.0))
	var row := HBoxContainer.new()
	row.alignment = BoxContainer.ALIGNMENT_CENTER
	box.add_child(row)
	_button = SurvivalUiKit.button("Begin again", SurvivalUiKit.TITLE)
	_button.pressed.connect(_on_begin_again)
	row.add_child(_button)
	var hint := SurvivalUiKit.rich(SurvivalUiKit.SMALL)
	hint.text = "[color=#e8e4dc99]or %s[/color]" % SurvivalUiKit.kbd("R")
	hint.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	row.add_child(SurvivalUiKit.spacer(0.0))
	row.add_child(hint)
	visible = false
	if world != null:
		update(world, 0.0, {})
...
func _write(world) -> void:
	...
	_headline.text = SurvivalUiKit.death_headline(cause)
	_line.text = SurvivalUiKit.death_line(cause)
	...
	_run_line.text = "%s · day %d%s · %s survived" % [
		title, int(world.day), season_text, SurvivalUiKit.clock_text(float(world.time))]
