...
## kind -> the cell, in `SurvivalInventory.EQUIPMENT_KINDS` order.
...
func setup(pkg, world, _fu) -> void:
	...
	layer = LAYER
	kit = SurvivalUiKit.new(pkg, world.manifest if world != null else {})
	_root = SurvivalUiKit.make_root(kit.theme)
	add_child(_root)
	_build_hud_panel()
	_build_equipment()
	_build_hotbar()
...
func set_ui_scale(scale_factor: float) -> void:
	...
	SurvivalUiKit.apply_scale(self, _root, ui_scale)
	_reflow()
	...
	if kit != null:
		kit.install_cursors(SurvivalUiKit.display_scale())
...
## is the season's (`SurvivalHelpers.night_factor`'s curve), so a winter clock says
## dusk earlier, in the same hours.
static func clock_of(world) -> Dictionary:
	var phase := fposmod(float(world.day_phase), 1.0)
	var share := SurvivalHelpers.night_share_of(world)
	...
	if length <= 0.0:
		length = SurvivalDayCycleSystem.DEFAULT_DAY_LENGTH
	var word := "day"
	var next := "dusk"
	var target := dusk
	var glyph := "sun"
...
func _write_slot(world, cell: Control, slot: Variant, selected: bool, hovered: bool) -> void:
	...
	count.visible = n > 1
	count.text = str(n)
	var spec := SurvivalUiKit.item_spec(world.manifest, item_id)
...
func _equipment_signature(world) -> String:
	var parts := PackedStringArray()
	var equipment: Dictionary = _equipment(world)
	for key: String in SurvivalInventory.EQUIPMENT_KINDS:
...
func _write_card(world, playing: bool) -> void:
	...
	if entry == null:
		...
		_card_name.add_theme_color_override("font_color", SurvivalUiKit.MUTED)
	...
	_card_name.text = SurvivalUiKit.item_name(world.manifest, item_id) + ((" ×%d" % many) if many > 1 else "")
	_card_name.add_theme_color_override("font_color", SurvivalUiKit.TEXT)
	_card_hint.text = SurvivalUiKit.use_hint(spec, entry)
	if worn:
		_use_button.disabled = false
		_use_button.text = "Take off"
		_drop_button.visible = false
		return
	var verb := SurvivalUiKit.use_verb(spec)
...
# ===========================================================================
# SurvivalWorld labels, message, debug
# ===========================================================================
...
func _describe_target(world, block: Dictionary) -> String:
	...
	if bool(block.get("item", false)):
		var item_name := SurvivalUiKit.item_name(world.manifest, str(entity.get("item_id", "")))
		var many := ""
...
func _describe_idle(world, entity: Dictionary) -> String:
	...
	if kind == "item" or kind == "forage":
		return SurvivalUiKit.item_name(world.manifest, str(entity.get("item_id", "")))
	return ""
...
func _write_debug(world) -> void:
	...
	for child in _debug_grid.get_children():
		...
		child.queue_free()
	for row: Array in rows:
		var name_label := SurvivalUiKit.label(str(row[0]), SurvivalUiKit.SMALL, SurvivalUiKit.MUTED)
		_debug_grid.add_child(name_label)
		var value := SurvivalUiKit.label(str(row[1]), SurvivalUiKit.SMALL)
...
func _build_hud_panel() -> void:
	...
	_warm = SurvivalUiKit.label("", SurvivalUiKit.SMALL, SurvivalUiKit.ACCENT)
	box.add_child(_warm)
...
func _build_buttons() -> void:
	...
	_craft_button = SurvivalUiKit.button("Craft  C", SurvivalUiKit.SMALL)
	_craft_button.toggle_mode = true
	_craft_button.pressed.connect(_on_craft_button)
	column.add_child(_craft_button)
	_map_button = SurvivalUiKit.button("Map  M", SurvivalUiKit.SMALL)
	_map_button.pressed.connect(_on_map_button)
	column.add_child(_map_button)
	_menu_button = SurvivalUiKit.button("Menu  Esc", SurvivalUiKit.SMALL)
...
func _build_card() -> void:
	...
	_card_name = SurvivalUiKit.label("", SurvivalUiKit.FONT_SIZE)
	column.add_child(_card_name)
	_card_hint = SurvivalUiKit.label("", SurvivalUiKit.SMALL, SurvivalUiKit.MUTED)
	...
	_use_button = SurvivalUiKit.button("Use", SurvivalUiKit.SMALL)
	_use_button.pressed.connect(_on_use)
	buttons.add_child(_use_button)
	_drop_button = SurvivalUiKit.button("Drop", SurvivalUiKit.SMALL)
	_drop_button.pressed.connect(_on_drop)
	buttons.add_child(_drop_button)
	_card_keys = SurvivalUiKit.rich(SurvivalUiKit.SMALL)
	_card_keys.text = "[color=#e8e4dc80]%s %s[/color]" % [SurvivalUiKit.kbd("X"), SurvivalUiKit.kbd("Z")]
...
func _build_message() -> void:
	...
	_message = SurvivalUiKit.label("", SurvivalUiKit.FONT_SIZE)
	_message_panel.add_child(_message)
	_message.modulate.a = 0.0


func _build_labels() -> void:
	_hover_label = SurvivalUiKit.outlined(SurvivalUiKit.SMALL, SurvivalUiKit.TEXT)
	_hover_label.name = "hover_label"
	_hover_label.visible = false
	_root.add_child(_hover_label)
	_focus_label = SurvivalUiKit.outlined(SurvivalUiKit.FONT_SIZE, SurvivalUiKit.ACCENT)
...
func _build_debug() -> void:
	...
	_debug_status = SurvivalUiKit.label("", SurvivalUiKit.SMALL, SurvivalUiKit.MISSING)
	_debug_status.visible = false
	box.add_child(_debug_status)
	var hint := SurvivalUiKit.label("1 biomes · 2 weights · 3 mixer · 4 road · 0 off · J stochastic" \
		+ " · K wire · ⇧X prints · [ ] cutoff · - = zoom · I save", SurvivalUiKit.SMALL, SurvivalUiKit.MUTED)
...
func _make_cell(corner: String) -> SlotCell:
	...
	cell.add_child(icon)
	if corner != "":
		var key := SurvivalUiKit.label(corner, 11, SurvivalUiKit.MUTED)
		key.name = "key"
		...
		cell.add_child(key)
	var count := SurvivalUiKit.label("", SurvivalUiKit.SMALL, SurvivalUiKit.ACCENT)
	count.name = "count"
...
func _reflow() -> void:
	...
	SurvivalUiKit.fit(_hud_panel)
	_hud_panel.position = Vector2(MARGIN, MARGIN)

	SurvivalUiKit.fit(_equip_panel)
	SurvivalUiKit.fit(_hotbar_panel)
	SurvivalUiKit.fit(_buttons_panel)
	var gap := 10.0
	...
	SurvivalUiKit.fit(_message_panel)
	...
	SurvivalUiKit.fit(_debug_panel)
...
func _slot_capacity(world) -> int:
	return SurvivalUiKit.slot_capacity(world.manifest, _equipment(world), int(world.base_slots))
...
class BarView:
	...
	func _draw() -> void:
		...
		track.bg_color = SurvivalUiKit.BAR_BG
		track.border_color = SurvivalUiKit.BAR_BORDER
...
class DayStripView:
	...
	func _draw() -> void:
		...
		draw_rect(Rect2(tick_x - 1.0, 0.0, 2.0, size.y), SurvivalUiKit.TEXT)
...
class SlotCell:
	...
	func _draw() -> void:
		...
		box.bg_color = SurvivalUiKit.SWATCH if swatch else (Color("#1f2229") if hovered else SurvivalUiKit.BAR_BG)
		box.border_color = SurvivalUiKit.ACCENT if selected else (SurvivalUiKit.MUTED if hovered else SurvivalUiKit.BAR_BORDER)
		box.set_border_width_all(2 if selected else 1)
		box.set_corner_radius_all(5)
		draw_style_box(box, Rect2(Vector2.ZERO, size))
		if flash > 0.0:
			var glow := StyleBoxFlat.new()
			glow.bg_color = Color(SurvivalUiKit.ACCENT.r, SurvivalUiKit.ACCENT.g, SurvivalUiKit.ACCENT.b, 0.28 * flash)
			glow.border_color = Color(SurvivalUiKit.ACCENT.r, SurvivalUiKit.ACCENT.g, SurvivalUiKit.ACCENT.b, flash)
