class_name SurvivalMusic
extends Node

## The two loops the clock chooses between, ported from viewer/index.html
## section 5b (`class SurvivalMusic`, :4220-4653) — the music half of it; the one-shot
...
func _smooth(current: float, target: float, tau: float, dt: float) -> float:
	return SurvivalMusic.smooth(current, target, tau, dt)
