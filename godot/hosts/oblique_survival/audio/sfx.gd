class_name SurvivalSfx
extends Node

## The one-shot cues, the footsteps, the fire bed, the weather ambience and the
## thunder — the half of the viewer's `class SurvivalMusic` that is not the two music
...
## As `SurvivalMusic.auto_start`: nothing in Godot requires a gesture before audio.
...
func setup(pkg, world, _fu = null) -> void:
	...
	for index: int in VOICES:
		...
		voice.bus = SurvivalMusic.FOLEY_BUS
		add_child(voice)
		_voices.append(voice)
	if _streams.has("fire"):
		_fire = _loop_player("fire", _streams["fire"], SurvivalMusic.FOLEY_BUS)
	...
	if rain_sound.get("ambience") is Dictionary:
		...
		if stream != null:
			_rain_duration = float((rain_sound["ambience"] as Dictionary).get("duration_seconds",
				stream.get_length()))
			_ambience = _loop_player("rain", stream, SurvivalMusic.EFFECTS_BUS)
	if snow_sound.get("ambience") is Dictionary:
		...
		if stream != null:
			_wind = _loop_player("wind", stream, SurvivalMusic.EFFECTS_BUS)
	if rain_sound.get("strike") is Dictionary:
		...
		if _thunder_stream != null:
			...
			_thunder.bus = SurvivalMusic.EFFECTS_BUS
...
func start() -> void:
	...
	for player: AudioStreamPlayer in [_fire, _ambience, _wind]:
		if player != null:
			...
			SurvivalMusic.play_when_ready(player)
...
func _process(delta: float) -> void:
	if not started:
		return
	if _fire != null:
		_fire_gain = SurvivalMusic.smooth(_fire_gain, _fire_target, FIRE_TAU, delta)
		_fire.volume_linear = _fire_gain
	if _ambience != null:
		_rain_gain = SurvivalMusic.smooth(_rain_gain, _rain_target, RAIN_TAU, delta)
		_ambience.volume_linear = _rain_gain
	if _wind != null:
		_wind_gain = SurvivalMusic.smooth(_wind_gain, _wind_target, SNOW_TAU, delta)
...
func thunder(distance: float) -> void:
	...
	SurvivalMusic.play_when_ready(_thunder)
