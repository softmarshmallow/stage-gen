...
func run(h: TestHarness) -> void:
	...
	h.assert_false(masks.is_land(-250.0, -250.0), "the south-west corner is not water")
	h.assert_false(masks.is_land(250.0, 250.0), "the north-east corner is not water")
	...
	h.assert_eq(masks.biome_at(-18.0, -200.0), "dry_meadow", "biome at (-18, -200)")
	h.assert_near(masks.friction_at(-18.0, -200.0), 0.55, 1e-6, "friction on dry meadow")
	...
	var bare := SurvivalMasks.new()
	h.assert_true(bare.is_land(9999.0, -9999.0), "a run with no splat is not all land")
	h.assert_near(bare.friction_at(0.0, 0.0), SurvivalMasks.DEFAULT_FRICTION, 1e-9, "default friction")
