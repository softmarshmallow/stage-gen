...
func _flash_envelope(h: TestHarness) -> void:
	...
	h.assert_near(SurvivalWeatherView.flash_envelope(0.12, 0.5), 0.9, 1e-9, "flash at 0.12 s is the second pulse")
	# 0.9 * (1 - (0.4 - 0.16) / (0.5 - 0.16))
	h.assert_near(SurvivalWeatherView.flash_envelope(0.4, 0.5), 0.264705882, 1e-6, "flash at 0.4 s is on the tail")
	h.assert_eq(SurvivalWeatherView.flash_envelope(-0.01, 0.5), 0.0, "no flash before the strike")
	h.assert_eq(SurvivalWeatherView.flash_envelope(0.51, 0.5), 0.0, "no flash after the flash seconds")
	h.assert_eq(SurvivalWeatherView.flash_envelope(0.5, 0.5), 0.0, "the tail reaches zero at the last instant")
	# The shared uniform is the envelope at this gain (viewer :5534).
	h.assert_near(SurvivalWeatherView.FLASH_GAIN, 0.85, 1e-9, "u_flash is the envelope times 0.85")
...
func _pools(h: TestHarness) -> void:
	...
	if not h.assert_true(pkg != null, "the run package opened"):
		return
	var weather: SurvivalWeatherView = SurvivalWeatherView.new()
...
## is written in `SurvivalWeatherView.update`).
func _wet_ownership(h: TestHarness) -> void:
	...
	var decals: SurvivalDecals = SurvivalDecals.new()
	...
	var world: SurvivalWorld = TestFixtures.world()
	if world != null:
...
func _fire_cycle(h: TestHarness) -> void:
	...
	if pkg == null:
		return
	var fire: SurvivalFire = SurvivalFire.new()
...
## sparkles that land with it come from `SurvivalPuffs`, which is offered the same event
## (index.html :4173-4202, :5586). The bolt draws; it never throws the burst.
func _strike_bolt(h: TestHarness) -> void:
	var pkg: HostRunDir = h.package()
	if pkg == null:
		return
	var strikes: SurvivalStrikes = SurvivalStrikes.new()
	...
	h.assert_eq(material.shader.resource_path, SurvivalStrikes.SHADER_PATH,
	...
	h.assert_near(float(material.get_shader_parameter("u_opacity")),
		SurvivalStrikes.flash_envelope(0.07, seconds), 1e-9, "the bolt rides the flash envelope")
	strikes.update_strikes(seconds + 0.01)
	h.assert_eq(strikes._bolts.size(), 0, "the bolt is dropped once the flash is over")
	strikes.free()

	# The sparkle is the SurvivalPuffs module's, twelve of them, and only on a strike.
	var puffs: SurvivalPuffs = SurvivalPuffs.new()
