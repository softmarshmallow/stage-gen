...
func _stage(w: SurvivalWorld, x: float, z: float) -> Dictionary:
	...
	var hound := TestFixtures.mob(w, "m1", "grub_hound", x, z)
	w.entities.append(hound)
	return hound


func _approaches_and_hits(h: TestHarness, w: SurvivalWorld) -> void:
	...
	for i in 78:
		...
		if w.player.health < 100.0 and not bit:
			...
			h.assert_near(w.player.invulnerable, 0.7 - TestFixtures.STEP, 1e-6,
	...
	var hurt := TestFixtures.events_of(w, "hurt")
...
func _invulnerability_spaces_the_blows(h: TestHarness, w: SurvivalWorld) -> void:
	...
	h.assert_eq(TestFixtures.events_of(w, "hurt").size(), 0, "and raises nothing")
	h.assert_true(str(hound["state"]) != "", "the hound still has a state")


func _out_of_aggro_it_wanders(h: TestHarness, w: SurvivalWorld) -> void:
