...
##
## The formulas are exercised through `SurvivalVitalsSystem.update` directly so that one
...
func _reset(w: SurvivalWorld, season_id: String) -> void:
	...
	w.dead = false
	w.message = ""
	TestFixtures.force_season(w, season_id)


func _run_seconds(w: SurvivalWorld, seconds: float) -> void:
	var steps := int(round(seconds / TestFixtures.STEP))
	for i in steps:
		SurvivalVitalsSystem.update(w, TestFixtures.STEP)


func _hunger(h: TestHarness, w: SurvivalWorld) -> void:
...
func _a_day_of_hunger_starves(h: TestHarness, w: SurvivalWorld) -> void:
...
func _winter_cold(h: TestHarness, w: SurvivalWorld) -> void:
...
func _the_dark_is_cold(h: TestHarness, w: SurvivalWorld) -> void:
...
func _a_fire_gives_warmth_back(h: TestHarness, w: SurvivalWorld) -> void:
...
func _freezing_takes_health(h: TestHarness, w: SurvivalWorld) -> void:
	_reset(w, "winter")
	w.player.warmth = 0.0
	_run_seconds(w, 5.0)
...
func _death_fires_once(h: TestHarness, w: SurvivalWorld) -> void:
	_reset(w, "winter")
	w.player.warmth = 0.0
	w.player.health = 3.0
	_run_seconds(w, 10.0)
	h.assert_true(w.dead, "the cold finished the player")
	var deaths := TestFixtures.events_of(w, "death")
	...
	_run_seconds(w, 10.0)
	h.assert_eq(TestFixtures.events_of(w, "death").size(), 1, "and never again")
	...
	var starved := TestFixtures.events_of(w, "death")
