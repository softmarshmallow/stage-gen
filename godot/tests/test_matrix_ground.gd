...
func _t22_land_share(h: TestHarness, pkg: HostRunDir) -> void:
	var splat := pkg.image("package/world/splat.png")
	if splat == null:
		return
	var bytes := splat.get_data()
...
func _t23_run_frictions(h: TestHarness, pkg: HostRunDir) -> void:
	...
	h.assert_near(bare.friction_at(0.0, 0.0), SurvivalMasks.DEFAULT_FRICTION, 1e-9, "no plate, default friction")
	h.assert_near(SurvivalMasks.DEFAULT_FRICTION, 0.6, 1e-9, "the default friction is 0.6")
