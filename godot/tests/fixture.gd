...
## per caller, and builders for the entity shapes `SurvivalWorld.create` produces.
...
static func world() -> SurvivalWorld:
	var pkg := package()
	if pkg == null:
		return null
...
static func bare(w: SurvivalWorld) -> void:
	w.entities.clear()
	w.slots.clear()
...
static func release(w: SurvivalWorld) -> void:
	w.input["x"] = 0.0
	w.input["z"] = 0.0
...
## A prop entity in the shape `SurvivalWorld.create` builds (viewer 484-503).
static func prop(w: SurvivalWorld, id: String, prop_id: String, state: String, x: float, z: float) -> Dictionary:
...
## A mob entity in the shape `SurvivalWorld.create` builds (viewer 503-512).
static func mob(w: SurvivalWorld, id: String, actor_id: String, x: float, z: float) -> Dictionary:
...
## A forage entity in the shape `SurvivalWorld.create` builds (viewer 516-530).
static func forage(w: SurvivalWorld, id: String, cell_index: int, x: float, z: float) -> Dictionary:
...
static func force_season(w: SurvivalWorld, season_id: String) -> void:
...
static func events_of(w: SurvivalWorld, type_name: String) -> Array:
	var found: Array = []
