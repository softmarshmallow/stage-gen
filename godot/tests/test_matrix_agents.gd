...
func _reset(world: SurvivalWorld, season_id: String) -> void:
	...
	world.time = 0.0
	TestFixtures.force_season(world, season_id)


func _vitals(world: SurvivalWorld, seconds: float) -> void:
...
func _t12_hunger(h: TestHarness, world: SurvivalWorld) -> void:
	...
	var deaths := TestFixtures.events_of(world, "death")
...
func _t13_freezing_latch(h: TestHarness, world: SurvivalWorld) -> void:
...
func _t24_wander_pace(h: TestHarness, world: SurvivalWorld) -> void:
	...
	world.player.x = 60.0
	world.player.z = 60.0
	var hound := TestFixtures.mob(world, "m1", "grub_hound", 0.0, 0.0)
	# Off its home mark, so the orbit has somewhere to pull it.
	hound["x"] = 2.0
	hound["z"] = 0.0
	world.entities.append(hound)
	SurvivalMobAiSystem.update(world, STEP)
	...
	world.player.x = 0.0
	world.player.z = 0.0
	var chaser := TestFixtures.mob(world, "m2", "grub_hound", 4.0, 0.0)
...
func _t24_the_sea_turns_it_back(h: TestHarness, world: SurvivalWorld) -> void:
	var speed := 2.6
	...
	world.player.z = z
	var hound := TestFixtures.mob(world, "m1", "grub_hound", x, z)
	world.entities.append(hound)
	SurvivalMobAiSystem.update(world, STEP)
...
func _t24_the_blow_lands_at_the_end(h: TestHarness, world: SurvivalWorld) -> void:
	var states: Dictionary = ((world.manifest["actors"] as Dictionary)["grub_hound"] as Dictionary)["states"]
	var swing := SurvivalTargeting.state_duration(states["attack"])
	...
	world.player.x = 0.0
	world.player.z = 0.0
	var hound := TestFixtures.mob(world, "m1", "grub_hound", 1.5, 0.0)
	world.entities.append(hound)
	SurvivalMobAiSystem.update(world, STEP)
...
func _shore_point(world: SurvivalWorld, reach: float) -> Dictionary:
	var z := -240.0
	while z <= 240.0:
		var x := -240.0
		while x < 240.0:
...
## Records what `SurvivalSfx.footsteps` would have played, so the cadence can be read
## without an audio device or the run's clips.
class StepSpy extends SurvivalSfx:
	var calls: Array = []
...
func _t27_footsteps(h: TestHarness) -> void:
	h.assert_near(SurvivalSfx.WALK_RATE, 9.5, 1e-9, "the walk rate")
