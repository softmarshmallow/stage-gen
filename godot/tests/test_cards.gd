...
## builds the real `SurvivalCards` against the real run -- every mesh, material and
...
func _prop_cards(h: TestHarness, manifest: Dictionary) -> void:
	...
	var mesh := SurvivalCards.card_mesh(float(layout["width"]), float(layout["height"]), float(layout["foot"]))
	...
	h.assert_false(SurvivalCards.has_look(pine, "grown", "autumn"), "pine grown has no autumn look")

	# A state with no look falls back to summer, and so does an empty look.
	var summer := SurvivalCards.state_spec(pine, "grown", "")
	...
	for prop_id: String in props.keys():
		...
		for state: String in (prop.get("states", {}) as Dictionary).keys():
			var spec := SurvivalCards.state_spec(prop, state, "")
			var card := SurvivalCards.card_layout(spec)
...
func _actor_cards(h: TestHarness, manifest: Dictionary) -> void:
	...
	var layout := SurvivalCards.card_layout(front, int(front["columns"]))
	...
	for index: int in front["canonical_frame_indices"]:
		var window := SurvivalCards.frame_uv(index, columns, rows)
	...
	h.assert_eq(SurvivalCards.frame_uv(0, 4, 1), Vector4(0.0, 0.0, 0.25, 1.0), "frame 0 window")
	h.assert_eq(SurvivalCards.frame_uv(3, 4, 1), Vector4(0.75, 0.0, 0.25, 1.0), "frame 3 window")
	...
	for facing: String in (idle["facings"] as Dictionary).keys():
		...
		widths[facing] = SurvivalCards.card_layout(spec, int(spec["columns"]))["width"]
...
func _items(h: TestHarness, manifest: Dictionary) -> void:
	...
	var layout := SurvivalCards.card_layout(log_item)
	...
	var mesh := SurvivalCards.card_mesh(float(layout["width"]), float(layout["height"]), 1.0)
...
func _playback(h: TestHarness, manifest: Dictionary) -> void:
	...
	h.assert_eq(int(SurvivalCards.motion_frame(walk, 0.0)["frame"]), 0, "loop at t=0")
	h.assert_eq(int(SurvivalCards.motion_frame(walk, 0.124)["frame"]), 0, "loop just before the first flip")
	h.assert_eq(int(SurvivalCards.motion_frame(walk, 0.125)["frame"]), 1, "loop at the first flip")
	h.assert_eq(int(SurvivalCards.motion_frame(walk, 0.5)["frame"]), 0, "loop wraps after four frames")
	h.assert_false(bool(SurvivalCards.motion_frame(walk, 9.0)["done"]), "a loop is never done")
	...
	h.assert_eq(int(SurvivalCards.motion_frame(gather, 10.0)["frame"]), 3, "once clamps at the last frame")
	h.assert_true(bool(SurvivalCards.motion_frame(gather, 10.0)["done"]), "once is done past the end")
	h.assert_false(bool(SurvivalCards.motion_frame(gather, 0.0)["done"]), "once is not done at the start")

	# hold, and a one-frame strip, both freeze on the first index.
	h.assert_eq(int(SurvivalCards.motion_frame({"mode": "hold", "canonical_frame_indices": [2, 3]}, 5.0)["frame"]), 2, "hold")
	h.assert_true(bool(SurvivalCards.motion_frame({"mode": "loop", "canonical_frame_indices": [7]}, 5.0)["done"]), "one frame is done")
	...
	h.assert_eq(int(SurvivalCards.motion_frame(driven, 0.0, 0.0)["frame"]), 0, "driven at 0")
	h.assert_eq(int(SurvivalCards.motion_frame(driven, 0.0, 0.5)["frame"]), 2, "driven at half")
	h.assert_eq(int(SurvivalCards.motion_frame(driven, 0.0, 1.0)["frame"]), 3, "driven clamps at 1")
...
func _facing(h: TestHarness) -> void:
	...
	h.assert_eq(SurvivalCards.facing_for(1.0, 0.0, 0.0), "right", "yaw 0, +x")
	h.assert_eq(SurvivalCards.facing_for(-1.0, 0.0, 0.0), "left", "yaw 0, -x")
	h.assert_eq(SurvivalCards.facing_for(0.0, 1.0, 0.0), "front", "yaw 0, +z")
	h.assert_eq(SurvivalCards.facing_for(0.0, -1.0, 0.0), "back", "yaw 0, -z")

	# The side wins on a perfect diagonal: |sx| >= |sy| picks left/right.
	h.assert_eq(SurvivalCards.facing_for(1.0, 1.0, 0.0), "right", "the side wins the diagonal")
	h.assert_eq(SurvivalCards.facing_for(-1.0, -1.0, 0.0), "left", "the side wins the other diagonal")

	# A heading shorter than 0.05 keeps the facing the card already had.
	h.assert_eq(SurvivalCards.facing_for(0.01, 0.0, 0.0, "back"), "back", "a still card keeps its facing")
	h.assert_eq(SurvivalCards.facing_for(0.0, 0.0, 0.0), "front", "with no current facing, front")
	...
	h.assert_eq(SurvivalCards.facing_for(1.0, 0.0, yaw), "right", "yaw 45, +x")
	h.assert_eq(SurvivalCards.facing_for(0.0, 1.0, yaw), "left", "yaw 45, +z is a diagonal the side wins")
	h.assert_eq(SurvivalCards.facing_for(0.0, -1.0, yaw), "right", "yaw 45, -z")
	h.assert_eq(SurvivalCards.facing_for(-1.0, -1.0, yaw), "back", "yaw 45, away from the camera")
	h.assert_near(SurvivalCards.screen_right_component(1.0, 0.0, yaw), cos(yaw), EPS, "screen right component")
	h.assert_near(SurvivalCards.toward_camera_component(0.0, 1.0, yaw), cos(yaw), EPS, "toward camera component")
	h.assert_near(SurvivalCards.screen_right(0.0).x, 1.0, EPS, "screen right at yaw 0 is +x")
	h.assert_near(SurvivalCards.screen_right(0.0).y, 0.0, EPS, "screen right at yaw 0 has no z")
...
func _windows(h: TestHarness) -> void:
	...
	# the port keeps.
	h.assert_eq(SurvivalCards.frame_uv(0, 2, 2), Vector4(0.0, 0.0, 0.5, 0.5), "row 0 is the TOP row")
	h.assert_eq(SurvivalCards.frame_uv(2, 2, 2), Vector4(0.0, 0.5, 0.5, 0.5), "row 1 is the bottom row")
	h.assert_eq(SurvivalCards.frame_uv(3, 2, 2), Vector4(0.5, 0.5, 0.5, 0.5), "reading order")
	h.assert_eq(SurvivalCards.frame_uv(0, 0, 0), Vector4(0.0, 0.0, 1.0, 1.0), "a degenerate sheet is the whole image")
...
func _shadows(h: TestHarness, manifest: Dictionary) -> void:
	...
	h.assert_near(SurvivalShadows.shadow_profile(0.0), 0.92, 1e-12, "the profile at the centre")
	h.assert_near(SurvivalShadows.shadow_profile(1.0), 0.0, 1e-12, "the profile at the edge")
	h.assert_near(SurvivalShadows.shadow_profile(0.6), 0.2 * (1.0 - 0.6), 1e-12, "past the second ellipse")
	h.assert_true(SurvivalShadows.shadow_profile(0.3) > SurvivalShadows.shadow_profile(0.5), "the profile falls outward")
	...
	h.assert_near(
		float(SurvivalShadows.SHADOW_STRENGTH["skirt_decal"]) * float(blend["shadow_strength"]), 0.42, 1e-12,
		"props draw their contact at 0.42",
	)
	h.assert_near(
		float(SurvivalShadows.SHADOW_STRENGTH["shadow"]) * float(blend["shadow_strength"]), 0.7, 1e-12,
	...
	var image := SurvivalShadows.shadow_image(16)
	...
	h.assert_near(SurvivalShadows.shadow_profile(0.0), 0.92, 1e-12, "the stack at dead centre")
...
func _decals(h: TestHarness, manifest: Dictionary) -> void:
	...
	h.assert_near(SurvivalDecals.to_linear(1.0), 1.0, 1e-12, "sRGB 1 is linear 1")
...
func _playback_modes(h: TestHarness, manifest: Dictionary) -> void:
	...
	for key: String in expected.keys():
		...
		if mode == "once":
			...
			var at_end := SurvivalCards.motion_frame(spec, float(frames) / fps)
			h.assert_eq(int(at_end["frame"]), last, "%s holds the last frame" % key)
			h.assert_true(bool(at_end["done"]), "%s reports done" % key)
			h.assert_eq(int(SurvivalCards.motion_frame(spec, 30.0)["frame"]), last, "%s still holds it later" % key)
		else:
			# A loop is back on the first cell exactly one strip later.
			h.assert_eq(
				int(SurvivalCards.motion_frame(spec, float(frames) / fps)["frame"]),
				int((spec["canonical_frame_indices"] as Array)[0]),
				"%s wraps" % key,
			)
			h.assert_false(bool(SurvivalCards.motion_frame(spec, 30.0)["done"]), "%s never ends" % key)
...
func _module(h: TestHarness, pkg: HostRunDir) -> void:
	...
	for entity: Variant in world.entities:
		...
		var spec := SurvivalCards.state_spec(prop, String((entity as Dictionary)["state"]), "winter")
...
func _pick_follows_the_billboard(h: TestHarness, pkg: HostRunDir) -> void:
	...
	var drawn := SurvivalCards.drawn_basis(chosen_node, camera)
...
func _look_geometry(h: TestHarness, manifest: Dictionary) -> void:
	...
	for prop_id: String in props.keys():
		var prop: Dictionary = props[prop_id]
		for state: String in (prop.get("states", {}) as Dictionary).keys():
			if not SurvivalCards.has_look(prop, state, "winter"):
				continue
			compared += 1
			var summer := SurvivalCards.card_layout(SurvivalCards.state_spec(prop, state, ""))
			var winter := SurvivalCards.card_layout(SurvivalCards.state_spec(prop, state, "winter"))
			...
			var summer_mesh := SurvivalCards.card_mesh(
				float(summer["width"]), float(summer["height"]), float(summer["foot"]))
			var winter_mesh := SurvivalCards.card_mesh(
