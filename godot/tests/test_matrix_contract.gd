...
## `orderSystems` tags. Written out here rather than read from `SurvivalSim` so a
...
func _with_kind(pkg: HostRunDir, kind: String) -> Dictionary:
	var m := pkg.manifest.duplicate()
	m["kind"] = kind
	return m


func _with_scale(pkg: HostRunDir, height: Variant) -> Dictionary:
	var m := pkg.manifest.duplicate()
...
func _with_ground_size(pkg: HostRunDir, size: float) -> Dictionary:
	var m := pkg.manifest.duplicate()
...
func _with_actor_state(pkg: HostRunDir, key: String, value: Variant) -> Dictionary:
	var m := pkg.manifest.duplicate()
...
func _with_prop_state(pkg: HostRunDir, key: String, value: Variant) -> Dictionary:
	var m := pkg.manifest.duplicate()
...
func _with_prop(pkg: HostRunDir, key: String, value: Variant) -> Dictionary:
	var m := pkg.manifest.duplicate()
	var props := _props_copy(pkg)
	var pine: Dictionary = (props["pine"] as Dictionary).duplicate()
	pine[key] = value
...
func _props_copy(pkg: HostRunDir) -> Dictionary:
...
func _broken_props(pkg: HostRunDir) -> Dictionary:
	var props: Dictionary = {}
	for id: String in (pkg.manifest["props"] as Dictionary).keys():
...
## law itself is `SurvivalSim.advance`, which is what a headless run can measure.
func _t30_advance(h: TestHarness, pkg: HostRunDir) -> void:
	var cases := {0.0: 1, 0.004: 1, 1.0 / 60.0: 1, 0.025: 2, 0.5: 30, 2.0: 120}
	for seconds: float in cases.keys():
		var world := _bare(pkg)
		var before := world.time
		SurvivalSim.advance(world, seconds)
		var steps := int(round((world.time - before) / SurvivalSim.FIXED_STEP))
	...
	var world := _bare(pkg)
	SurvivalSim.advance(world, -5.0)
	h.assert_near(world.time, SurvivalSim.FIXED_STEP, 1e-12, "advance(-5) did not run its one step")

	# One substep per call at the fixed step, and the frame accumulator is
	# spent rather than kept.
	var ticked := _bare(pkg)
	h.assert_eq(SurvivalSim.tick(ticked, SurvivalSim.FIXED_STEP), 1, "one fixed step is one substep")
	h.assert_near(ticked.accumulator, 0.0, 1e-9, "the accumulator kept a remainder")
	h.assert_eq(SurvivalSim.tick(ticked, SurvivalSim.FIXED_STEP * 0.5), 0, "half a step ran a substep")
	h.assert_eq(SurvivalSim.tick(ticked, SurvivalSim.FIXED_STEP * 0.6), 1, "the remainder did not add up")
	# A stall drops time instead of spiralling: at most MAX_SUBSTEPS a frame.
	var stalled := _bare(pkg)
	h.assert_eq(SurvivalSim.tick(stalled, 10.0), SurvivalSim.MAX_SUBSTEPS, "a stalled frame ran more than the cap")
