...
func _onset_cut(h: TestHarness) -> void:
	...
	if stream == null:
		...
		return
	if SurvivalSfx.decode(stream).is_empty():
		h.note("no audio server here: the onset cut could not be measured")
		return
	var slices: Array = SurvivalSfx.cut_at_onsets(stream)
	...
	for slice: Array in slices:
		...
		h.assert_true(float(slice[1]) >= SurvivalSfx.MIN_SLICE_SECONDS, "and is at least the floor length")
	...
	var authored: Array = SurvivalSfx.cut_at_onsets(stream, [0.0, 1.0])
...
func _fade_gains(h: TestHarness) -> void:
	...
	h.assert_near(SurvivalMusic.curve_value("linear", 0.25), 0.25, 1e-9, "linear curve")
	h.assert_near(SurvivalMusic.curve_value("exponential", 0.25), 0.0625, 1e-9, "exponential curve")
	h.assert_near(SurvivalMusic.curve_value("nonsense", 1.0), 1.0, 1e-9, "an unknown curve is equal power")
	music.free()
