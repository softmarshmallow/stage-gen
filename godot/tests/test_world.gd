...
func _player(h: TestHarness, pkg: HostRunDir, world: SurvivalWorld) -> void:
...
func _pack(h: TestHarness, world: SurvivalWorld) -> void:
...
func _clock_and_conditions(h: TestHarness, world: SurvivalWorld) -> void:
	...
	h.assert_eq(world.input, SurvivalWorld.fresh_input(), "the input bag")
...
func _start_kit(h: TestHarness, pkg: HostRunDir) -> void:
	var kit := HostRunDir.new()
...
func _entity(world: SurvivalWorld, id: String) -> Dictionary:
	for entity: Dictionary in world.entities:
		if entity["id"] == id:
			return entity
	return {}
