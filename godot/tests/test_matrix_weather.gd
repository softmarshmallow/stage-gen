...
## clock. `SurvivalWeatherSystem.update` is driven directly wherever the assertion is
## about the slew alone, so a step is exactly one authored increment and the
## count of them is the measurement; the strike run goes through `SurvivalSim.step`
...
func _t7_rain_slew(h: TestHarness, pkg: HostRunDir) -> void:
	var world := _bare(pkg)
	...
	for _i in int(round(RAIN_ONSET * 0.5 / STEP)):
		SurvivalWeatherSystem.update(halfway, STEP)
	...
	SurvivalWeatherSystem.update(held, STEP)
...
## Steps `SurvivalWeatherSystem` until `done` answers true, or gives up after two minutes
## of simulated time. Returns the number of steps taken, or -1.
func _slew_steps(world: SurvivalWorld, done: Callable) -> int:
	for i in int(round(120.0 / STEP)):
		SurvivalWeatherSystem.update(world, STEP)
		if bool(done.call()):
			return i + 1
	return -1
...
func _t8_snow_is_a_season(h: TestHarness, pkg: HostRunDir) -> void:
	var world := _bare(pkg)
	TestFixtures.force_season(world, "winter")
	# A handful of things to watch the look swap reach.
	var pine := TestFixtures.prop(world, "p1", "pine", "grown", 2.0, 0.0)
	var rock := TestFixtures.prop(world, "r1", "moss_boulder", "whole", -2.0, 0.0)
	var moss := TestFixtures.forage(world, "f1", 11, 0.0, 1.0)
	var hound := TestFixtures.mob(world, "m1", "grub_hound", 8.0, 8.0)
...
func _t9_no_rain_under_snow(h: TestHarness, pkg: HostRunDir) -> void:
	var world := _bare(pkg)
	TestFixtures.force_season(world, "winter")
	...
	TestFixtures.force_season(summer, "summer")
	summer.weather["mode"] = "auto"
	summer.weather["spell_ends_at"] = -1.0
	SurvivalWeatherSystem.update(summer, STEP)
...
func _t10_strike_now(h: TestHarness, pkg: HostRunDir) -> void:
	var world := _bare(pkg)
	...
	for _i in 64:
		...
		world.events.clear()
		SurvivalWeatherSystem.strike_now(world, strike)
		var events := TestFixtures.events_of(world, "strike")
		if events.size() != 1:
			thunder_ok = false
			break
...
func _t10_strikes_in_a_storm(h: TestHarness, pkg: HostRunDir) -> void:
	var world := _bare(pkg)
	world.weather["mode"] = "storm"
	var strikes: Array = []
	var thunder: Array = []
