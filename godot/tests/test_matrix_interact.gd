...
func _t17_dead_snag(h: TestHarness, world: SurvivalWorld) -> void:
	...
	world.dead = false
	var snag := TestFixtures.prop(world, "s1", "dead_snag", "standing", 0.0, 1.0)
	world.entities.append(snag)
	SurvivalInventory.inv_add(world, "axe", 1)
	var target: Variant = SurvivalTargeting.target_for(world, snag)
	h.assert_eq(int((target as Dictionary)["hits"]), 2, "an axe takes the snag in two blows")

	var seen: Array = []
	for _i in 200:
		world.input["interact"] = true
		SurvivalSim.step(world, STEP)
	...
	h.assert_eq(TestFixtures.events_of(world, "hit").size(), 2, "two blows were struck")
...
func _t18_fell_sign(h: TestHarness, world: SurvivalWorld) -> void:
	...
	h.assert_near(SurvivalInteractSystem.FALL_SECONDS, 1.1, 1e-9, "the fall takes 1.1 s")
	...
	SurvivalSim.advance(world, 1.0)
	h.assert_eq(_items(world).size(), 0, "the yield fell early")
	SurvivalSim.advance(world, 0.2)
...
func _fell(world: SurvivalWorld, px: float, pz: float, tree_x: float, tree_z: float, yaw: float) -> Dictionary:
	var pine := _stage_pine(world, px, pz, tree_x, tree_z, yaw)
	_swing(world, pine)
	var fell := TestFixtures.events_of(world, "fell")
...
func _stage_pine(world: SurvivalWorld, px: float, pz: float, tree_x: float, tree_z: float, yaw: float) -> Dictionary:
	TestFixtures.bare(world)
	...
	world.dead = false
	world.drops.clear()
	var pine := TestFixtures.prop(world, "p1", "pine", "grown", tree_x, tree_z)
	world.entities.append(pine)
	SurvivalInventory.inv_add(world, "axe", 1)
	return pine


func _swing(world: SurvivalWorld, pine: Dictionary) -> void:
	for _i in 400:
		world.input["interact"] = true
		SurvivalSim.step(world, STEP)
...
func _items(world: SurvivalWorld) -> Array:
	var found: Array = []
...
func _t19_draw_order(h: TestHarness, world: SurvivalWorld) -> void:
	TestFixtures.bare(world)
	...
	world.drop_count = 0
	world.rng = KernelRng.new(4242)
	world.rand = Callable(world.rng, "next")
	var reference := KernelRng.new(4242)
	...
	SurvivalDropsSystem.spawn_drops(world, [{"item_id": "log", "count": 1}], 3.0, -4.0, 1.0, 0.0, 1.7)
	...
	SurvivalDropsSystem.spawn_drops(world, [{"item_id": "log", "count": 4}], 0.0, 0.0, 1.0, 0.0, 1.0)
	var counted := KernelRng.new(0)
...
func _t19_flight(h: TestHarness, world: SurvivalWorld) -> void:
	h.assert_near(SurvivalDropsSystem.DROP_GRAVITY, 26.0, 1e-9, "the cartoon gravity")
	h.assert_near(SurvivalDropsSystem.DROP_RESTITUTION, 0.28, 1e-9, "the bounce coefficient")
	h.assert_near(SurvivalDropsSystem.SLIDE_GRAVITY, 9.81, 1e-9, "the g the slide brakes against")
	h.assert_near(SurvivalDropsSystem.DROP_SETTLE_SPEED, 0.06, 1e-9, "the settle speed")

	# Gravity: one step off the ground costs 26 m/s of upward speed a second.
	TestFixtures.bare(world)
	var flying := _item(world, 0.0, 0.0, 2.0, 0.0, 0.0, 0.0)
	SurvivalDropsSystem.update(world, STEP)
	h.assert_near(float(flying["vy"]), -SurvivalDropsSystem.DROP_GRAVITY * STEP, 1e-9, "one step of gravity")
	h.assert_near(float(flying["y"]), 2.0 - SurvivalDropsSystem.DROP_GRAVITY * STEP * STEP, 1e-9,
	...
	h.assert_near(float(hard["vy"]), (-6.0 - SurvivalDropsSystem.DROP_GRAVITY * STEP) * -SurvivalDropsSystem.DROP_RESTITUTION,
	...
	SurvivalDropsSystem.update(world, STEP)
	h.assert_near(float(sliding["vx"]), 3.0 - friction * SurvivalDropsSystem.SLIDE_GRAVITY * STEP, 1e-9,
		"one step of the biome's brake")


func _item(world: SurvivalWorld, x: float, z: float, y: float, vx: float, vz: float, vy: float) -> Dictionary:
	var item := {
