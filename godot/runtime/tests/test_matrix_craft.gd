...
func _every_recipe(h: TestHarness, world: SurvivalWorld) -> void:
	...
	for recipe: Dictionary in recipes:
		...
		for item_id: String in ingredients.keys():
			SurvivalInventory.inv_add(world, item_id, int(ingredients[item_id]))
		var status := SurvivalCraftSystem.recipe_status(world, recipe)
		if not bool(status["ok"]):
			failed.append("%s (status)" % str(recipe["recipe_id"]))
			continue
		if not SurvivalCraftSystem.craft_recipe(world, recipe):
		...
		if prop_id != null and str(prop_id) != "":
			...
			SurvivalCraftSystem.update(world, STEP)
			SurvivalSim.clear_one_shots(world)
		# The cost is gone.
		for item_id: String in ingredients.keys():
			if SurvivalInventory.count(world, item_id) != 0:
		...
		if prop_id != null and str(prop_id) != "":
			...
			if str(built["state"]) != str(product["state"]):
				...
				continue
		elif SurvivalInventory.count(world, str(product["item_id"])) + _worn_count(world, str(product["item_id"])) != int(product.get("count", 1)):
	...
	SurvivalInventory.inv_add(world, "twig", 1)
	h.assert_false(SurvivalCraftSystem.craft_recipe(world, axe_recipe), "an axe was made out of one twig")
	h.assert_eq(world.message, "Need 1 Flint.", "the shortfall is named")
	h.assert_eq(SurvivalInventory.count(world, "twig"), 1, "and the twig was not spent")
	...
	_stage(world)
	SurvivalInventory.inv_add(world, "grass_tuft", 10)
	SurvivalInventory.inv_add(world, "moss", 10)
	SurvivalInventory.inv_add(world, "log", 100)
	h.assert_eq(SurvivalInventory.slot_capacity(world), 12, "the pack is twelve slots")
	var empty := 0
	for slot: Variant in world.slots:
		if slot == null:
			empty += 1
	h.assert_eq(empty, 0, "the pack was not full before the overflow test")
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, _recipe(world, "poultice")), "the poultice was not made")
	h.assert_eq(SurvivalInventory.count(world, "poultice"), 0, "the poultice went into a full pack")
	var dropped := 0
...
func _station_reach(h: TestHarness, world: SurvivalWorld) -> void:
	...
	# No fire at all.
	_stage(world)
	SurvivalInventory.inv_add(world, "berry", 2)
	h.assert_false(SurvivalCraftSystem.craft_recipe(world, cooked), "berries were stewed with no fire")
	h.assert_eq(world.message, "Needs a campfire within reach.", "and the refusal names the station")
	h.assert_eq(SurvivalInventory.count(world, "berry"), 2, "the berries were not spent")
	...
	SurvivalInventory.inv_add(world, "berry", 2)
	_fire(world, 3.3, "lit")
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, cooked), "a fire 2.926 m away was refused")
	h.assert_eq(SurvivalInventory.count(world, "cooked_berry"), 1, "the stew was not made")

	# One step further out and the edge is past the reach.
	_stage(world)
	SurvivalInventory.inv_add(world, "berry", 2)
	_fire(world, 3.5, "lit")
	h.assert_false(SurvivalCraftSystem.craft_recipe(world, cooked), "a fire 3.126 m away was accepted")
	h.assert_eq(SurvivalInventory.count(world, "berry"), 2, "and nothing was spent")

	# The pick wants a workbench, and says so.
	_stage(world)
	SurvivalInventory.inv_add(world, "flint", 2)
	SurvivalInventory.inv_add(world, "twig", 2)
	var pickaxe: Dictionary = _recipe(world, "pickaxe")
	h.assert_false(SurvivalCraftSystem.craft_recipe(world, pickaxe), "a pick was made with no workbench")
	h.assert_eq(world.message, "Needs a workbench within reach.", "the refusal names the workbench")
	h.assert_eq(SurvivalInventory.count(world, "flint"), 2, "the flint was not spent")
	_put_station(world, "workbench")
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, pickaxe), "with a workbench the pick is made")
	...
	h.assert_eq(SurvivalInventory.count(world, "pickaxe"), 0, "not left in the pack")


## The campfire station wants the `lit` state; the workbench authors none, so
## any state serves.
func _station_state(h: TestHarness, world: SurvivalWorld) -> void:
	var cooked: Dictionary = _recipe(world, "cooked_berry")
	_stage(world)
	SurvivalInventory.inv_add(world, "berry", 2)
	var fire := _fire(world, 2.0, "unlit")
	h.assert_false(SurvivalCraftSystem.craft_recipe(world, cooked), "berries were stewed on a cold fire")
	h.assert_eq(SurvivalInventory.count(world, "berry"), 2, "and nothing was spent")
	fire["state"] = "lit"
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, cooked), "lighting the same fire did not help")
...
func _build_and_place(h: TestHarness, world: SurvivalWorld) -> void:
	...
	var stand_off := radius + world.player.radius + SurvivalCraftSystem.PLACE_STAND_OFF
	h.assert_near(stand_off, 0.374 + 0.34 + 0.45, 1e-9, "the stand-off distance")

	_stage(world)
	SurvivalInventory.inv_add(world, "grass_tuft", 3)
	SurvivalInventory.inv_add(world, "log", 2)
	...
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, campfire), "the fire did not go to the pointer")
	...
	h.assert_eq(SurvivalInventory.count(world, "log"), 2, "nothing is spent yet")
	...
	h.assert_eq(str(placing["why"]), SurvivalCraftSystem.WHY_FAR, "because it is out of reach")
	world.input["place_click"] = true
	SurvivalCraftSystem.update(world, STEP)
	SurvivalSim.clear_one_shots(world)
	...
	h.assert_eq(SurvivalInventory.count(world, "log"), 2, "and nothing was spent")
	world.entities.append(TestFixtures.prop(world, "t1", "canvas_tent", "pitched", 2.0, 0.0))
	world.input["place_point"] = {"x": 2.0, "z": 0.0}
	SurvivalCraftSystem.update(world, STEP)
	SurvivalSim.clear_one_shots(world)
	h.assert_eq(str(placing["why"]), SurvivalCraftSystem.WHY_ROOM, "a tent's footprint refuses the spot")
	world.input["place_point"] = {"x": 0.0, "z": 0.0}
	SurvivalCraftSystem.update(world, STEP)
	SurvivalSim.clear_one_shots(world)
	h.assert_eq(str(placing["why"]), SurvivalCraftSystem.WHY_ROOM, "and so does the player's own")
	# Water is not ground either, judged at the spot (the far corner is sea).
	h.assert_eq(SurvivalCraftSystem.place_verdict(world, -250.0, -250.0, radius), SurvivalCraftSystem.WHY_FAR, "the sea is far from here")
	world.player.x = -249.5
	world.player.z = -249.5
	h.assert_eq(SurvivalCraftSystem.place_verdict(world, -250.0, -250.0, radius), SurvivalCraftSystem.WHY_WATER, "and water when it is near")
	world.player.x = 0.0
	world.player.z = 0.0
	...
	h.assert_eq(SurvivalInventory.count(world, "log"), 0, "the logs are spent at the set-down")
	h.assert_eq(SurvivalInventory.count(world, "grass_tuft"), 0, "and the grass")
	h.assert_eq(world.message, "Campfire built.", "and it is said")
	h.assert_eq(TestFixtures.events_of(world, "craft").size(), 1, "one craft event, at the set-down")

	# Building draws exactly one value, for the card's seed; a look no `light`
	# interaction leads to does not burn.
	_stage(world)
	world.rng = KernelRng.new(99)
	world.rand = Callable(world.rng, "next")
	var reference := KernelRng.new(99)
	var expected_seed := int(reference.next() * 1e5)
	var seeded: Variant = SurvivalCraftSystem.place_prop(world, "campfire", "unlit", 0.0, 2.0)
...
func _a_refused_build_spends_nothing(h: TestHarness, world: SurvivalWorld) -> void:
	_stage(world)
	...
	world.entities.append(TestFixtures.prop(world, "t1", "canvas_tent", "pitched", 0.0, 0.0))
	SurvivalInventory.inv_add(world, "grass_tuft", 3)
	SurvivalInventory.inv_add(world, "log", 2)
	var campfire: Dictionary = _recipe(world, "campfire")
	world.message = ""
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, campfire), "the build did not go to the pointer")
	if not h.assert_true(world.placing != null, "nothing came to the pointer"):
		return
	var placing := world.placing as Dictionary
	h.assert_false(bool(placing["ok"]), "the spot in front is taken, so the silhouette is red")
	h.assert_eq(str(placing["why"]), SurvivalCraftSystem.WHY_ROOM, "for want of room")
	world.input["place_click"] = true
	SurvivalCraftSystem.update(world, STEP)
	SurvivalSim.clear_one_shots(world)
	h.assert_eq(world.message, "No room there.", "a click there says why")
	h.assert_eq(world.built, 0, "and nothing was built")
	h.assert_eq(SurvivalInventory.count(world, "grass_tuft"), 3, "the grass was not spent")
	h.assert_eq(SurvivalInventory.count(world, "log"), 2, "nor the logs")
	...
	h.assert_eq(SurvivalInventory.count(world, "grass_tuft"), 3, "the grass is still there")
	h.assert_eq(SurvivalInventory.count(world, "log"), 2, "and the logs")
	# Asking for the table again lets a build go too.
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, campfire), "the build did not go to the pointer again")
	...
	h.assert_eq(SurvivalInventory.count(world, "log"), 2, "with the logs kept")
...
func _a_made_thing_is_worn_when_its_place_is_free(h: TestHarness, world: SurvivalWorld) -> void:
	_stage(world)
	SurvivalInventory.inv_add(world, "twig", 1)
	SurvivalInventory.inv_add(world, "flint", 1)
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, _recipe(world, "axe")), "the axe was not made")
	var hand: Variant = world.equipment["hand"]
	h.assert_true(hand is Dictionary and str((hand as Dictionary)["item"]) == "axe",
		"the made axe went straight into the empty hand")
	h.assert_eq(SurvivalInventory.count(world, "axe"), 0, "and not into the pack")
	h.assert_eq(world.message, "Made %s, now in hand." % SurvivalInventory.item_name(world, "axe"), "and it is said")
	# A second axe, the hand taken, stays in the pack.
	SurvivalInventory.inv_add(world, "twig", 1)
	SurvivalInventory.inv_add(world, "flint", 1)
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, _recipe(world, "axe")), "the second axe was not made")
	h.assert_eq(SurvivalInventory.count(world, "axe"), 1, "the second axe stays in the pack")
	h.assert_eq(world.message, "Made %s." % SurvivalInventory.item_name(world, "axe"), "said plainly")
	# A pack goes on the back.
	_put_station(world, "workbench")
	SurvivalInventory.inv_add(world, "rope", 4)
	SurvivalInventory.inv_add(world, "grass_tuft", 4)
	h.assert_true(SurvivalCraftSystem.craft_recipe(world, _recipe(world, "backpack")), "the pack was not made")
	...
	h.assert_eq(world.message, "Made %s, now on the back." % SurvivalInventory.item_name(world, "backpack"), "and it is said")
...
func _the_menu(h: TestHarness, world: SurvivalWorld) -> void:
	var recipes: Array = (world.manifest["crafting"] as Dictionary)["recipes"]
	_stage(world)
	world.input["craft_toggle"] = true
	SurvivalCraftSystem.update(world, STEP)
...
func _recipe(world: SurvivalWorld, id: String) -> Dictionary:
...
func _fire(world: SurvivalWorld, distance: float, state: String) -> Dictionary:
	var fire := TestFixtures.prop(world, "f1", "campfire", "unlit", 0.0, distance)
	fire["state"] = state
...
func _worn_count(world: SurvivalWorld, item_id: String) -> int:
	var n := 0
...
func _built_prop(world: SurvivalWorld, prop_id: String) -> Dictionary:
	for entity: Dictionary in world.entities:
