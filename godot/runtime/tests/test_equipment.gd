...
func _inputs_are_declared(h: TestHarness, w: SurvivalWorld) -> void:
	...
	h.assert_eq(SurvivalInventory.EQUIPMENT_KINDS, ["hand", "body", "back"], "in the HUD's order")


func _use_wears_a_cloak(h: TestHarness, w: SurvivalWorld) -> void:
	_fresh(w)
	...
	h.assert_near(SurvivalInventory.insulation(w), 0.5, 1e-9, "and the cold is halved")
	...
	h.assert_eq(SurvivalInventory.slot_capacity(w), 16, "and the pack is sixteen slots")
	h.assert_eq(w.slots.size(), 16, "grown on the spot")


func _equip_and_unequip_inputs(h: TestHarness, w: SurvivalWorld) -> void:
	_fresh(w)
	...
	h.assert_eq(SurvivalInventory.count(w, "axe"), 1, "back into the pack")
	h.assert_eq(w.message, "Flint axe off.", "and says so")
	w.input["unequip"] = "nowhere"
	SurvivalSim.step(w, TestFixtures.STEP)
	h.assert_eq(SurvivalInventory.count(w, "axe"), 1, "an unknown place is ignored")
	# A worn thing with no room to come off stays on.
	_fresh(w)
	SurvivalInventory.inv_add(w, "grass_cloak", 1)
	SurvivalInventory.equip(w, 0)
	SurvivalInventory.inv_add(w, "log", 120)
...
func _the_worn_tool_serves_first(h: TestHarness, w: SurvivalWorld) -> void:
	_fresh(w)
	SurvivalInventory.inv_add(w, "axe", 1)
	SurvivalInventory.inv_add(w, "axe", 1)
	SurvivalInventory.equip(w, 1)
	var pine := TestFixtures.prop(w, "p1", "pine", "grown", 0.0, 0.5)
	...
	h.assert_eq(int((target as Dictionary)["tool_slot"]), SurvivalInventory.HAND_SLOT, "served by the hand")
...
func _a_fresh_world_is_dry(h: TestHarness) -> void:
	...
	SurvivalSim.advance(world, 30.0)
	...
	SurvivalSim.advance(world, dry_low)
