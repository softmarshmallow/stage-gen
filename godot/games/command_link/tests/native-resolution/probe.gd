...
func _run() -> void:
	...
	for configuration in [
		...
		picture.save_png("res://tests/native-resolution/" + str(configuration[0]) + ".png")
	...
	FileAccess.open("res://tests/native-resolution/probe.json", FileAccess.WRITE).store_string(JSON.stringify(result, "\t"))
