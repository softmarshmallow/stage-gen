...
## Run with -s res://tests/composition_checks.gd, with or without --headless.
...
func _game_roots() -> void:
	var registry: Script = load("res://roots.gd")
	...
	var laboratory: RefCounted = registry.call("create", "lab")
	_expect(command_link != null and laboratory != null, "Both explicit roots must resolve.")
	_expect(registry.call("create", "bishoujo_afterlight") == null, "This project registers no other game.")
	if command_link == null or laboratory == null:
		return
	...
	if command_scene == null or companion == null:
		return
	...
	_observations.append("distinct mission and lab roots, unchanged Command Link entry, and separate study ownership")
