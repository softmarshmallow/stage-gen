...
func _run() -> void:
	...
	_expect(changed, "The mission's opening option must not prevent a Lab detour.")
	if changed:
		await process_frame
	...
	if _issues.is_empty(): print("PASS route options: Command Link variant b -> Lab -> Command Link variant b; content root and original options preserved.")
