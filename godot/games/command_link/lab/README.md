...
Command Link's own laboratory: focused feature controls on the tactical
fixture, with the mission paused. Afterlight's studies live in
[Afterlight's lab](../../afterlight/lab/). Historical text below describes the
shared Lab before the split.

```sh
Godot --path godot/games/command_link -- --game lab --language ko
```
...
The [current project inventory](../../../packages/game_presentation/history/CURRENT_STATUS.md) records remaining
...
host and route UI live under [afterlight/](../../afterlight/lab/); the earlier tactical
workbenches remain under [demos/](./). These are demo integrations,
...
compatibility. New links should use `--game lab` explicitly.
...
```sh
Godot --path godot/games/command_link -- --game lab --route actor_motion_study --language ko
```
...
```sh
Godot --path godot/games/command_link -- --game lab --route cast_pan_study --language ko
```
...
```sh
Godot --path godot/games/command_link -- --game lab --route sigh_puff_study --language ko
```
...
```sh
Godot --path godot/games/command_link -- --game lab --route ominous_study
```
...
```sh
Godot --path godot/games/command_link -- --game lab --route sprite_burst_study --language ko
```
...
```sh
Godot --path godot/games/command_link -- --game lab --route background_blackout_study --language ko
```
...
```sh
Godot --path godot/games/command_link -- --game lab --route ambient_particles_study --language ko
```
...
```sh
Godot --path godot/games/command_link -- --game lab --route transmission_voice_study --language ko
```
