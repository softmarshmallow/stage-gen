...
## Focused native proof of the shared TV shader on an actor and a framed feed.
const LEGACY = preload("res://qa/legacy_presentation.gd")
...
const OUTPUT := "res://qa/hologram-defaults"
...
func _run() -> void:
	...
	if _errors.is_empty(): print("PASS hologram defaults: native 1x/2x tactical actor and framed Eira feed, 90%/70% defaults, zero-strength pixel bypass, temporal animation, alpha outline, independent controls, and unchanged pixels outside the intended actor/feed")
...
func _transmission(width: int) -> void:
	...
	await _settle()
	_expect(app.open_route("game:bishoujo_afterlight/new_game"), "The actual Afterlight host must open.")
	await _settle()
