...
func run(root: Control, options: Dictionary) -> void:
	...
	if _errors.is_empty() and options.has("capture-routes"):
		if DisplayServer.get_name() == "headless":
			...
			await _capture_routes(root, String(options.get("capture-dir", "res://qa/routes/captures")))
...
func _menu_demo(root: Control, demo_id: String) -> void:
	if root.selected_game_id == "command_link":
		...
		await _settle(root)
		_expect(root.selected_game_id == "presentation_lab", "The mission menu must open the dedicated laboratory game.")
...
func _resume_and_demos(root: Control) -> void:
	...
	for demo_id: String in DEMOS:
		...
		_key(root, KEY_ENTER)
		await _settle(root)
		_expect(root.selected_game_id == "presentation_lab" and _route(root) == "command_link", "Demo Back must return to the laboratory collection: " + demo_id)
		await _menu_demo(root, demo_id)
