...
const CONTACT_OUTPUT := "res://qa/contact-afterlight"
...
func _run() -> void:
	...
	var sources := {}
	for path: String in ["res://addons/game_presentation/interaction/point_contact.gd", "res://games/bishoujo_afterlight/root.gd", "res://games/bishoujo_afterlight/story.gd", "res://games/bishoujo_afterlight/story_beats.gd", "res://games/bishoujo_afterlight/text/en.json", "res://games/bishoujo_afterlight/text/ko.json"]:
		sources[path] = FileAccess.get_sha256(path)
	var portrait_path := "res://games/bishoujo_afterlight/assets/characters/nami_contact.png"
...
func _seek_contact() -> Control:
	_expect(_app.open_route("game:bishoujo_afterlight/new_game"), "A fresh Afterlight episode must open.")
	await _settle()
	var game := _active()
