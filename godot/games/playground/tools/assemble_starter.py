...
#: Where a linked SDK must resolve: the package payload two tiers above a game project.
PACKAGE_PAYLOAD = Path("packages/game_presentation") / SDK_PATH
EXCLUDED = {".godot", ".git", "__pycache__", ".DS_Store", ".gdignore"}


def _sdk_payload(project: Path) -> Path:
    """The SDK directory to copy. A game links `addons/game_presentation` to the
    package payload under `godot/packages/`; the link is followed only when it
    points exactly there, and the payload itself is then a real directory."""
    sdk = project / SDK_PATH
    if not sdk.is_symlink():
        return sdk
    if len(project.parents) < 2:
        raise ValueError(f"SDK link cannot resolve to a package payload: {sdk}")
    expected = (project.parents[1] / PACKAGE_PAYLOAD).resolve()
    resolved = sdk.resolve()
    if resolved != expected or not resolved.is_dir():
        raise ValueError(f"SDK link must point at the package payload {expected}: {sdk}")
    return resolved
