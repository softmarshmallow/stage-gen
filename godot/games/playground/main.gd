...
## One application shell; each game root supplies its own scenes and content.
const GAMES = preload("res://main_games.gd")
const DEMOS = preload("res://demos/root.gd")
...
func _ready() -> void:
	...
	game_root = GAMES.create(selected_game_id)
	if game_root == null:
		printerr("Unknown game. Use --game command_link, bishoujo_afterlight, or presentation_lab.")
		get_tree().quit(2)
		return
	...
	if options.has("validate") or options.has("capture-state") or options.has("capture-all"):
		var stage: Control = load("res://qa/legacy_presentation.gd").new()
...
func open_route(route_id: String) -> bool:
	...
	if destination.begins_with("game:"):
		...
		target_id = "presentation_lab"
		destination = "command_link"
	elif destination.begins_with("demos/") or destination in ["approach_study", "eye_study", "effects_menu", "intertitle_study", "drift_study", "halo_study"]:
		# Preserve old launch links, but make the laboratory the actual owner.
		target_id = "presentation_lab"
	if target_id == "dating_sim":
		target_id = "bishoujo_afterlight"
	if target_id == "bishoujo_afterlight" and destination == "menu":
	...
	if target_root == null:
		target_root = GAMES.create(target_id)
	if target_root == null:
		printerr("Unknown game: " + target_id)
		return false
	# Opening selection belongs to Command Link. Keep the original launch
	# options so returning to that game retains its selected variant.
...
func _run_route_checks() -> void:
	var checks = load("res://qa/route_checks.gd").new()
	await checks.run(self, options)


func _run_actor_focus_checks() -> void:
	var checks = load("res://qa/actor_focus_checks.gd").new()
	await checks.run(self, options)


func _run_manpu_animation_checks() -> void:
	var checks = load("res://qa/manpu_animation_checks.gd").new()
	await checks.run(self, options)


func _run_character_exit_checks() -> void:
	var checks = load("res://qa/character_exit_checks.gd").new()
	await checks.run(self, options)


func _run_cast_transition_checks() -> void:
	var checks = load("res://qa/cast_transition_checks.gd").new()
	await checks.run(self, options)


func _run_establishing_shot_checks() -> void:
	var checks = load("res://qa/establishing_shot_checks.gd").new()
	await checks.run(self, options)


func _run_dialogue_camera_checks() -> void:
	var checks = load("res://qa/dialogue_camera_checks.gd").new()
	await checks.run(self, options)
