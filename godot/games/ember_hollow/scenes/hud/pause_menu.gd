...
func _build_menu(world) -> void:
	...
	_menu.add_child(box)
	var headline := SurvivalUiKit.label("Paused", HEADLINE, SurvivalUiKit.ACCENT)
	...
	var sub := SurvivalUiKit.label(title, SurvivalUiKit.SMALL, SurvivalUiKit.MUTED)
	sub.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	box.add_child(sub)
	box.add_child(SurvivalUiKit.spacer(8.0))
	for pair in [
		["Resume", "resume", "Esc"],
		["How to play", "help", ""],
		["Map", "map", "M"],
		...
		var button := SurvivalUiKit.button(str(pair[0]), SurvivalUiKit.FONT_SIZE)
	...
	var hint := SurvivalUiKit.rich(SurvivalUiKit.SMALL)
	hint.text = "[center][color=#e8e4dc99]%s or %s resumes[/color][/center]" % [SurvivalUiKit.kbd("Esc"), SurvivalUiKit.kbd("P")]
	box.add_child(hint)
...
func _build_help() -> void:
	...
	var title := SurvivalUiKit.label("How to play", SurvivalUiKit.TITLE, SurvivalUiKit.ACCENT)
	title.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	title.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	header.add_child(title)
	var back := SurvivalUiKit.button("Back", SurvivalUiKit.SMALL)
	back.pressed.connect(show_page.bind("menu"))
	header.add_child(back)
	for section in help_sections():
		var heading := SurvivalUiKit.label(str(section[0]), SurvivalUiKit.FONT_SIZE, SurvivalUiKit.ACCENT)
	...
	return [
		...
		["The pack",
			...
			+ "pack to wear it; click the worn thing to take it off.") % [
				SurvivalUiKit.kbd("hand"), SurvivalUiKit.kbd("body"), SurvivalUiKit.kbd("back")]],
		["Crafting",
			...
			+ "far), right-click to keep the makings. A fire is built lit.") % [
				SurvivalUiKit.kbd("C"), SurvivalUiKit.kbd("Enter")]],
		["Keys",
			("%s / arrows move · %s %s turn · %s interact · %s light a fire · %s–%s select · "
			% [SurvivalUiKit.kbd("WASD"), SurvivalUiKit.kbd("Q"), SurvivalUiKit.kbd("E"), SurvivalUiKit.kbd("Space"), SurvivalUiKit.kbd("F"),
				SurvivalUiKit.kbd("1"), SurvivalUiKit.kbd("0")])
			+ ("%s use or wear · %s drop · %s craft · %s map · %s this menu · %s begin again · %s fullscreen"
			% [SurvivalUiKit.kbd("X"), SurvivalUiKit.kbd("Z"), SurvivalUiKit.kbd("C"), SurvivalUiKit.kbd("M"), SurvivalUiKit.kbd("Esc"),
				SurvivalUiKit.kbd("R"), SurvivalUiKit.kbd("F11")])],
		["The clock",
		...
		["Dev keys",
			...
			% [SurvivalUiKit.kbd("G"), SurvivalUiKit.kbd("V"), SurvivalUiKit.kbd("N"), SurvivalUiKit.kbd("K"), SurvivalUiKit.kbd("T"),
				SurvivalUiKit.kbd("L"), SurvivalUiKit.kbd("B")])
			+ ("%s %s zoom · %s pause · %s debug panel" % [
				SurvivalUiKit.kbd("-"), SurvivalUiKit.kbd("="), SurvivalUiKit.kbd("P"), SurvivalUiKit.kbd("`")])],
	]
