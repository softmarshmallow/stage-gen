...
func set_ui_scale(scale_factor: float) -> void:
	SurvivalUiKit.apply_scale(self, _root, scale_factor)
	if open:
		_layout()
...
func _build(world) -> void:
	...
	var title := SurvivalUiKit.label(str((world.manifest as Dictionary).get("title", "The land")), SurvivalUiKit.TITLE, SurvivalUiKit.ACCENT)
	_column.add_child(title)
	_column.add_child(SurvivalUiKit.label("%d m across · north up" % int(size_meters), SurvivalUiKit.SMALL, SurvivalUiKit.MUTED))
	_column.add_child(SurvivalUiKit.spacer(6.0))
	...
	_column.add_child(SurvivalUiKit.spacer(14.0))
	var closing := SurvivalUiKit.rich(SurvivalUiKit.SMALL, COLUMN_WIDTH)
	closing.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	closing.text = "%s or %s closes the map, or click anywhere on it." % [SurvivalUiKit.kbd("M"), SurvivalUiKit.kbd("Esc")]
...
func _legend_entry(label: String, colour: Color) -> Control:
	...
	var text := SurvivalUiKit.label(label, SurvivalUiKit.SMALL, SurvivalUiKit.TEXT)
...
class MapView:
	...
	func _to_map(point: Vector2) -> Vector2:
		return Vector2(
			((point.x + size_meters * 0.5) / size_meters) * SurvivalWorldMap.CANVAS,
			((point.y + size_meters * 0.5) / size_meters) * SurvivalWorldMap.CANVAS)

	func _draw() -> void:
		var box := Rect2(0.0, 0.0, SurvivalWorldMap.CANVAS, SurvivalWorldMap.CANVAS)
		if base != null:
			draw_texture_rect(base, box, false)
		else:
			draw_rect(box, SurvivalWorldMap.WATER)
		...
		for i: int in steps + 1:
			var angle := heading - SurvivalWorldMap.WEDGE_HALF_ANGLE \
				+ (2.0 * SurvivalWorldMap.WEDGE_HALF_ANGLE) * (float(i) / float(steps))
			wedge.append(p + Vector2(cos(angle), sin(angle)) * SurvivalWorldMap.WEDGE_RADIUS)
		draw_colored_polygon(wedge, Color(1.0, 1.0, 1.0, 0.38))
		draw_circle(p, SurvivalWorldMap.PLAYER_RADIUS, Color.WHITE)
		draw_arc(p, SurvivalWorldMap.PLAYER_RADIUS, 0.0, TAU, 24, Color("#111111"), 1.5)
		...
		var bar := (meters / size_meters) * SurvivalWorldMap.CANVAS
		draw_rect(Rect2(10.0, SurvivalWorldMap.CANVAS - 30.0, bar + 12.0, 20.0), Color(0.0, 0.0, 0.0, 0.45))
		draw_rect(Rect2(16.0, SurvivalWorldMap.CANVAS - 17.0, bar, 3.0), Color.WHITE)
		if font != null:
			draw_string(font, Vector2(16.0, SurvivalWorldMap.CANVAS - 21.0), "%d m" % int(meters),
