...
func setup(pkg, _world, fu) -> void:
	...
	uniforms = fu
	# `SurvivalCards` owns the material and mesh maths; gallery mode only arranges what
	# it makes, so it borrows one instance rather than copying the builders.
	_cards = SurvivalCards.new()
...
func build() -> void:
	...
	for row: Dictionary in rows(manifest):
		...
		for cell: Dictionary in row["cells"]:
			...
			var layout := SurvivalCards.card_layout(spec, columns)
			var material := _cards.card_material(
				package.texture(String(spec.get("atlas", spec.get("image", "")))),
				bool(found["soft"]),
			)
			if columns > 1:
				material.set_shader_parameter("u_frame_uv", SurvivalCards.frame_uv(0, columns, 1))
...
func _post(height: float, z: float, title: String) -> void:
	...
	var mesh := SurvivalCards.card_mesh(POST_WIDTH, height, 1.0)
	mesh.surface_set_material(0, material)
	var node := MeshInstance3D.new()
	node.mesh = mesh
...
## gallery draws at the world origin. Same rule as `SurvivalCards._attach`.
