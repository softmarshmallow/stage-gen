...
##
##   Godot --headless --path godot/games/ember_hollow -s res://tools/parity.gd \
##       --quit-after 100000 -- --run <absolute run dir> \
##       --script res://tests/fixtures/oblique_survival/replay/02-walk-and-chop.json --out <abs jsonl>
##
...
## the keys go through the same `SurvivalInput` sampler `main.gd` samples with, and
## the step is `SurvivalSim.step` at the fixed step — so a divergence is the
...
func _init() -> void:
	...
	# takes it off the machine's keyboard (`SurvivalInput.polling`): a headless run
	# must not read whatever is held down on the desk.
	var sampler := SurvivalInput.new()
	sampler.bind(world)
	...
	for item_id: String in give.keys():
		for _n in range(int(give[item_id])):
			if SurvivalInventory.inv_add(world, item_id, 1) > 0:
	...
	for step_index in range(1, total + 1):
		...
		SurvivalSim.step(world, FIXED_STEP)
...
static func _digest(world: SurvivalWorld, step_index: int, event_counts: Dictionary, messages: Array) -> Dictionary:
	...
	return {
		...
		"rng_next_u32": int(round(next_raw * KernelRng.DIVISOR)),
		"player": {
			"x": _r4(player.x),
			"z": _r4(player.z),
			"vx": _r4(player.vx),
