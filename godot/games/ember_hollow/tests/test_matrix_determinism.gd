...
func _stage(world: SurvivalWorld) -> void:
	...
	SurvivalInventory.inv_add(world, "pickaxe", 1)
	SurvivalInventory.inv_add(world, "berry", 4)
	SurvivalInventory.inv_add(world, "torch", 1)
...
func _hound(world: SurvivalWorld, id: String, x: float, z: float) -> Dictionary:
	var hound := TestFixtures.mob(world, id, "grub_hound", x, z)
...
func _script_input(world: SurvivalWorld, step: int) -> void:
	var leg := (step / 90) % 4
...
func _digest(world: SurvivalWorld) -> String:
...
func _t29_boot_budget(h: TestHarness, pkg: HostRunDir) -> void:
	var best := INF
	for _i in 3:
