...
func _fell_a_pine(h: TestHarness, w: SurvivalWorld) -> void:
	var pine := _stand_at_the_pine(w)
	SurvivalInventory.inv_add(w, "axe", 1)
	var before: Variant = SurvivalTargeting.target_for(w, pine)
	...
	h.assert_eq(TestFixtures.events_of(w, "hit").size(), 2, "two blows were struck")
	var fell := TestFixtures.events_of(w, "fell")
	...
	h.assert_true(SurvivalTargeting.target_for(w, pine) == null, "a stump offers no second chop")


func _refused_without_an_axe(h: TestHarness, w: SurvivalWorld) -> void:
	var pine := _stand_at_the_pine(w)
	...
	h.assert_eq(TestFixtures.events_of(w, "hit").size(), 0, "no blow landed")
...
func _the_snag_offers_what_the_pack_allows(h: TestHarness, w: SurvivalWorld) -> void:
	TestFixtures.bare(w)
	w.player.x = 0.0
	w.player.z = 0.0
	w.player.busy = null
	w.player.approach = null
	w.dead = false
	var snag := TestFixtures.prop(w, "s2", "dead_snag", "standing", 0.0, 0.8)
	w.entities.append(snag)
	var bare: Variant = SurvivalTargeting.target_for(w, snag)
	...
	SurvivalInventory.inv_add(w, "axe", 1)
	var armed: Variant = SurvivalTargeting.target_for(w, snag)
	...
	h.assert_eq(SurvivalUiKit.inv_count(w.slots, "twig"), 2, "and two twigs went into the pack")
	var broken: Variant = SurvivalTargeting.target_for(w, snag)
	...
	h.assert_true(SurvivalTargeting.target_for(w, snag) == null, "and a stump offers nothing")


func _barren_in_winter(h: TestHarness, w: SurvivalWorld) -> void:
	TestFixtures.bare(w)
	w.player.x = 0.0
	w.player.z = 0.0
	w.player.busy = null
	...
	var bush := TestFixtures.prop(w, "b1", "thorn_bush", "full", 0.0, 0.7)
	w.entities.append(bush)
	var target: Variant = SurvivalTargeting.target_for(w, bush)
	...
	h.assert_true((SurvivalTargeting.target_for(w, bush) as Dictionary)["disabled"] == null,
		"in summer the same bush is gatherable")


func _progress_looks(h: TestHarness, w: SurvivalWorld) -> void:
	...
	w.player.busy = null
	w.dead = false
	var boulder := TestFixtures.prop(w, "r1", "moss_boulder", "whole", 0.0, 0.85)
	w.entities.append(boulder)
	SurvivalInventory.inv_add(w, "pickaxe", 1)
	var seen: Array = []
	for i in 200:
		w.input["interact"] = true
		SurvivalSim.step(w, TestFixtures.STEP)
	...
	h.assert_eq(TestFixtures.events_of(w, "hit").size(), 3, "three blows with the pick")
	# `timers` runs after `interact` in the same step, so the clock has already
	# ticked once by the time the test looks at it.
	h.assert_near(float(boulder["regrow"]), 120.0, TestFixtures.STEP + 1e-6,
...
func _gathered_by_hand(h: TestHarness, w: SurvivalWorld) -> void:
	TestFixtures.bare(w)
	w.player.x = 0.0
	w.player.z = 0.0
	w.player.busy = null
	...
	var bush := TestFixtures.prop(w, "t1", "twig_bush", "full", 0.0, 0.5)
	w.entities.append(bush)
	for i in 60:
		w.input["interact"] = true
		SurvivalSim.step(w, TestFixtures.STEP)
	...
	h.assert_eq(SurvivalInventory.count(w, "twig"), 2, "both twigs are in the pack already")
	var on_ground := 0
	for entity in w.entities:
		if str((entity as Dictionary).get("kind", "")) == "item":
			on_ground += 1
	h.assert_eq(on_ground, 0, "and nothing fell to be picked up after")
	var pickups := TestFixtures.events_of(w, "pickup")
...
func _a_full_pack_lets_the_rest_fall(h: TestHarness, w: SurvivalWorld) -> void:
	TestFixtures.bare(w)
	w.player.x = 0.0
	w.player.z = 0.0
	w.player.busy = null
	...
	for i in 400:
		if SurvivalInventory.inv_add(w, "log", 1) > 0:
			break
	h.assert_true(SurvivalInventory.inv_add(w, "log", 1) == 1, "the pack is full of logs")
	var bush := TestFixtures.prop(w, "t2", "twig_bush", "full", 0.0, 0.5)
	w.entities.append(bush)
	w.message = ""
	for i in 60:
		w.input["interact"] = true
	...
	h.assert_eq(SurvivalInventory.count(w, "twig"), 0, "no twig fit")
	var fallen := 0
	for entity in w.entities:
...
func _the_key_waits_for_the_yield(h: TestHarness, w: SurvivalWorld) -> void:
	TestFixtures.bare(w)
	w.player.x = 0.0
	w.player.z = 0.0
	w.player.busy = null
	...
	TestFixtures.force_season(w, "summer")
	SurvivalInventory.inv_add(w, "pickaxe", 1)
	var boulder := TestFixtures.prop(w, "r2", "moss_boulder", "whole", 0.0, 0.85)
	w.entities.append(boulder)
	...
	var bush := TestFixtures.prop(w, "t3", "twig_bush", "full", 0.0, -1.0)
	w.entities.append(bush)
	...
	for i in 600:
		...
		for entity in w.entities:
			if str((entity as Dictionary).get("kind", "")) == "item" and not bool((entity as Dictionary)["settled"]):
				...
				break
		if SurvivalInventory.count(w, "stone") == 2:
			break
	h.assert_true(stones_flew, "the stones were seen in the air")
	h.assert_eq(SurvivalInventory.count(w, "stone"), 2, "both stones were taken")
...
func _take_a_forage_piece(h: TestHarness, w: SurvivalWorld) -> void:
	TestFixtures.bare(w)
	w.player.x = 0.0
	w.player.z = 0.0
	w.player.busy = null
	w.dead = false
	TestFixtures.force_season(w, "summer")
	var moss := TestFixtures.forage(w, "f1", 11, 0.0, 0.4)
	...
	h.assert_eq(SurvivalInventory.count(w, "moss"), 1, "and counted into the pack")
	h.assert_near(float(moss["regrow"]), 200.0, TestFixtures.STEP + 1e-6,
		"its regrow clock is the cell's")
	h.assert_eq(TestFixtures.events_of(w, "pickup").size(), 1, "one pickup was heard")
