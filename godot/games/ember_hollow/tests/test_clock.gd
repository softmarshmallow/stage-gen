...
func _night_curve(h: TestHarness) -> void:
	...
	h.assert_near(SurvivalHelpers.night_factor(0.50, 0.38), 0.0, 1e-9, "summer dusk begins")
	h.assert_near(SurvivalHelpers.night_factor(0.56, 0.38), 0.5, 1e-9, "summer half way into dusk")
	h.assert_near(SurvivalHelpers.night_factor(0.62, 0.38), 1.0, 1e-9, "summer full night")
	h.assert_near(SurvivalHelpers.night_factor(0.72, 0.38), 1.0, 1e-9, "summer deep night")
	h.assert_near(SurvivalHelpers.night_factor(0.88, 0.38), 1.0, 1e-9, "summer dawn begins")
	h.assert_near(SurvivalHelpers.night_factor(0.94, 0.38), 0.5, 1e-9, "summer half way through dawn")
	h.assert_near(SurvivalHelpers.night_factor(0.9999, 0.38), 0.0008333333, 1e-7, "summer day's turn")

	# Winter (night_share 0.55): dusk at 0.33.
	h.assert_near(SurvivalHelpers.night_factor(0.32, 0.55), 0.0, 1e-9, "winter before dusk")
	h.assert_near(SurvivalHelpers.night_factor(0.39, 0.55), 0.5, 1e-9, "winter half way into dusk")
	h.assert_near(SurvivalHelpers.night_factor(0.45, 0.55), 1.0, 1e-9, "winter full night")
	# The same phase is night in winter and day in summer: the share matters.
	h.assert_near(SurvivalHelpers.night_factor(0.40, 0.38), 0.0, 1e-9, "phase 0.40 is day in summer")
	h.assert_true(SurvivalHelpers.night_factor(0.40, 0.55) > 0.5, "phase 0.40 is not night in winter")

	# The dusk is clamped into [0.2, 0.76] however long the night is authored.
	h.assert_near(SurvivalHelpers.night_factor(0.19, 0.9), 0.0, 1e-9, "a very long night still starts at 0.2")
	h.assert_near(SurvivalHelpers.night_factor(0.26, 0.9), 0.5, 1e-9, "a very long night's dusk ramp")
	h.assert_near(SurvivalHelpers.night_factor(0.75, 0.0), 0.0, 1e-9, "a run with no night")
	h.assert_near(SurvivalHelpers.night_factor(0.82, 0.0), 0.5, 1e-9, "the shortest night's dusk ramp")

	# The phase wraps.
	h.assert_near(SurvivalHelpers.night_factor(1.72, 0.38), 1.0, 1e-9, "phase 1.72 wraps to 0.72")
	h.assert_near(SurvivalHelpers.night_factor(-0.28, 0.38), 1.0, 1e-9, "phase -0.28 wraps to 0.72")
...
func _calendar(h: TestHarness) -> void:
	...
	for index in expected.size():
		var day := index + 1
		var now := SurvivalHelpers.season_for(season, day)
		h.assert_eq(now["id"], expected[index], "day %d is not %s" % [day, expected[index]])
	h.assert_eq(SurvivalHelpers.season_for(season, 5)["day_in_season"], 1, "day 5 is the first of winter")
	h.assert_eq(SurvivalHelpers.season_for(season, 8)["day_in_season"], 4, "day 8 is the fourth of winter")
	h.assert_eq(SurvivalHelpers.season_for(season, 9)["index"], 0, "day 9 wraps the order")

	# A forced season overrides the id but not the index the calendar reached.
	season["force"] = "winter"
	h.assert_eq(SurvivalHelpers.season_for(season, 1)["id"], "winter", "a forced season did not take")
	h.assert_eq(SurvivalHelpers.season_for(season, 1)["index"], 0, "a forced season moved the index")

	# A run with no calendar has no season at all.
	h.assert_eq(SurvivalHelpers.season_for({"calendar": null, "specs": {}, "force": "auto"}, 3)["id"], "", "no calendar")
...
func _snow_onset_and_decay(h: TestHarness, pkg: HostRunDir) -> void:
	...
	SurvivalSim.advance(world, 55.0)
	...
	SurvivalSim.advance(world, 10.0)
