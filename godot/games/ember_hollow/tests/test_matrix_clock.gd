...
func _t5_night_curve(h: TestHarness) -> void:
	for row: Array in NIGHT_SUMMER:
		h.assert_near(SurvivalHelpers.night_factor(float(row[0]), 0.38), float(row[1]), 5e-5,
			"nightFactor(%.2f, 0.38)" % float(row[0]))
	for row: Array in NIGHT_WINTER:
		h.assert_near(SurvivalHelpers.night_factor(float(row[0]), 0.55), float(row[1]), 5e-5,
...
func _t26_fade_gains(h: TestHarness, pkg: HostRunDir) -> void:
