...
## Godot --path godot/games/afterlight --script res://tests/waking_eye_integration_checks.gd -- --game afterlight
## Add --capture-waking on a native renderer for 1x/2x story and laboratory frames.
const DESIGN_SIZE := Vector2(1280, 900)
const CAPTURE_DIRECTORY := "res://tests/waking-eye"
var _app: Control
...
func _run() -> void:
	...
	await _settle()
	_expect(_app.selected_game_id == "afterlight", "Run this fixture with --game afterlight.")
...
func _reach_opening() -> Control:
	_expect(_app.open_route("game:afterlight/new_game"), "A new Afterlight episode must open.")
	await _settle()
	for action in 8:
...
func _laboratory(window_size: Vector2i) -> void:
	...
	_expect(_app._game_states["afterlight"] == saved, "Lab tuning must not mutate the suspended story checkpoint.")
