...
func _episode_checks() -> void:
	...
	root.add_child(_app)
	await _settle()
	_expect(_app.selected_game_id == "afterlight", "Run this check with --game afterlight.")
	if _app.selected_game_id != "afterlight": return
	...
	_expect(_app.open_route("game:lab"), "Text playback must permit a real Lab detour.")
	await _settle()
	_expect(old_audio.get_ref() == null, "Leaving the episode must free its audio players.")
	_expect(_app.open_route("game:afterlight"), "The episode must restore after the Lab detour.")
	await _settle()
...
func _capture_typing_preview() -> void:
	...
	preview.data = pcm
	var directory := "res://tests/text-audio"
