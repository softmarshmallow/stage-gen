...
func _run() -> void:
	...
	for path: String in [DIRECTORY + "voice_policy.gd", DIRECTORY + "voices.json", DIRECTORY + "cast.json", DIRECTORY + "manifest.json", "res://story.gd", "res://root.gd", "res://story_beats.gd", "res://text/en.json", "res://text/ko.json", "res://addons/game_presentation/audio/text_reveal_audio.gd"]:
		if FileAccess.file_exists(path): _evidence["source_sha256"][path] = FileAccess.get_sha256(path)
	DirAccess.make_dir_recursive_absolute("res://tests/voiceovers")
	var file := FileAccess.open("res://tests/voiceovers/validation.json", FileAccess.WRITE)
...
func _policy_checks() -> void:
	...
	_sets = {"en": _json("res://text/en.json"), "ko": _json("res://text/ko.json")}
	...
	var recording := {"status": "ready", "source_revision": revision, "path": "res://tests/voiceovers/not-a-recording.mp3"}
...
func _host_checks() -> void:
	...
	_expect(_app.selected_game_id == "afterlight" and game._load_errors.is_empty(), "The real Afterlight host must initialize with prepared voice fixtures.")
	if _app.selected_game_id != "afterlight" or not game._load_errors.is_empty(): return
	...
	_expect(_app.open_route("game:lab"), "A voiced line must permit a real Lab detour.")
	await _settle()
	_expect(old_audio.get_ref() == null, "Leaving the story must free and stop its audio players.")
	_expect(_app.open_route("game:afterlight"), "The story must resume its voiced checkpoint after Lab.")
	await _settle()
	...
	_expect(_app.open_route("game:lab"), "A completed recording must also checkpoint through Lab.")
	await _settle()
	_expect(_app.open_route("game:afterlight"), "A completed recording must restore from Lab.")
	await _settle()
	game = _app.active_scene
...
func _removed_line_checks() -> void:
	...
	for item: Dictionary in _inventory:
		if item["line_id"] == "episode.no_forwarding_address":
			records["lines"][item["language"]][item["line_id"]] = {"status": "ready", "source_revision": item["source_revision"], "path": "res://tests/voiceovers/removed-recording.mp3"}
	var reduced: Array = []
...
func _voice_frame(label: String, game: Control) -> void:
	...
	var path := "res://tests/voiceovers/" + label + ".png"
