...
const APPROACH_OUTPUT := "res://tests/quick-approach"
...
func _run() -> void:
	...
	var sources := {}
	for source: String in ["res://cast_stage.gd", "res://root.gd", "res://story.gd", "res://story_beats.gd", "res://lab/actor_motion_study.gd"]:
...
func _laboratory_approach(factor: int) -> void:
	_expect(_app.open_route("game:lab/actor_motion_study"), "Quick Approach must be available in the existing actor-motion study.")
	await _settle()
	var lab := _active()
	lab.set_language("ko")
	...
	_expect(_app.open_route("game:afterlight"), "The motion study must return to the saved episode.")
	await _settle()
