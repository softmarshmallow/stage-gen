...
const ADAPTER = preload("res://content_adapter.gd")
...
func _cache_checks(game: Control, loader: RefCounted, directory: String) -> void:
	...
	var sets := {}
	for language: String in ["en", "ko"]:
		sets[language] = game.content.content_loader.read_json("text/" + language + ".json").value
	...
	for name: String in [voice_directory]:
