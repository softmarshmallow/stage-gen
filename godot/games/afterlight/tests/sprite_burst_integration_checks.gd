...
const BURST_OUTPUT := "res://tests/sprite-burst"
...
func _run() -> void:
	...
	var sources := {}
	for source: String in ["res://addons/game_presentation/effects/particles/sprite_burst.gd", "res://story.gd", "res://story_beats.gd", "res://root.gd", "res://lab/sprite_burst_study.gd"]:
...
func _laboratory_burst(factor: int) -> void:
	_expect(_app.open_route("game:lab/sprite_burst_study"), "The shared burst must have a dedicated laboratory route.")
	await _settle()
	var lab := _active()
	...
	_expect(_app.open_route("game:afterlight"), "The laboratory must return to the saved episode.")
	await _settle()
