extends "res://tests/autoplay_checks.gd"

## Real host integration; PCM/art outputs remain local and unapproved listening.
const PASS_OUTPUT := "res://tests/ambient-transmission"
...
func _run() -> void:
	...
	_expect(_app.selected_game_id == "afterlight" and game._load_errors.is_empty(), "Load the actual Afterlight host with --game afterlight.")
	if _errors.is_empty():
	...
	for path: String in ["res://story.gd", "res://root.gd", "res://atmosphere_profile.gd", "res://addons/game_presentation/effects/particles/ambient_particles.gd", "res://addons/game_presentation/audio/voice_effects.gd", "res://addons/game_presentation/audio/text_reveal_audio.gd", "res://tests/ambient_transmission_integration_checks.gd"]:
...
func _atmosphere_checks(game: Control) -> void:
	...
	_expect(_app.open_route("game:lab"), "A real Lab detour must open.")
	await _settle()
	_expect(_app.open_route("game:afterlight"), "Return from Lab must restore the story host.")
	await _settle()
	game = _app.active_scene
...
func _transmission_checks(game: Control) -> void:
	...
	_expect(_app.open_route("game:lab"), "Voice Lab detour must open.")
	await _settle()
	_expect(_app.open_route("game:afterlight"), "Voice Lab detour must restore.")
	await _settle()
	game = _app.active_scene
