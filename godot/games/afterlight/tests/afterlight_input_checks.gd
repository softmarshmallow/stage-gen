...
const DIRECTORY := "res://tests/afterlight-ensemble"
var _app: Control
...
func _choice_detour(game: Control, beat_id: String, selected: String) -> Control:
	...
	_expect(_app.open_route("game:lab"), "A choice checkpoint must permit a real Lab detour.")
	await _settle()
	_expect(_app.open_route("game:afterlight"), "The story must resume after its choice detour.")
	await _settle()
