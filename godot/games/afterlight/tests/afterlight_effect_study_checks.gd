...
## Godot --headless --path godot/games/afterlight --script res://tests/afterlight_effect_study_checks.gd
...
const CAPTURES := "res://tests/afterlight-effects"
...
func _run() -> void:
	...
	if _capture_enabled and DisplayServer.get_name() == "headless":
		...
		quit(2)
		return
	var composition = load("res://root.gd").new()
...
func _make_scene(route: String, content: Dictionary) -> Control:
	var packed = load("res://lab/" + route + ".tscn")
