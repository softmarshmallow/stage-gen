...
##
##   Godot --path godot/games/_shared/runtime -- --run <absolute run dir> \
...
static func parse(argv: PackedStringArray) -> HostArgs:
	var args := HostArgs.new()
	var index := 0
...
static func from_command_line() -> HostArgs:
...
func _to_string() -> String:
	return "HostArgs(run=%s mode=%s time=%s season=%s weather=%s seed=%d)" % [
