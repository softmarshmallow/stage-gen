"""Bounded v2 compiler admission, independent of production or media preparation."""
...
from .fixtures import DEFAULT_SCRIPT, declarations_value


def _admit(*, script: str = DEFAULT_SCRIPT, **overrides: Any) -> None:
    declarations = ScenarioDeclarations.model_validate(declarations_value(**overrides))
    admit_scenario(declarations, compile_scenario(declarations, parse_scenario(script)))


def test_the_default_narrative_is_admitted_and_reaches_every_ending() -> None:
    declarations = ScenarioDeclarations.model_validate(declarations_value())
    report = admit_scenario(
        declarations, compile_scenario(declarations, parse_scenario(DEFAULT_SCRIPT))
    )
    assert report.admitted is True
    assert {witness.outcome_id for witness in report.witnesses} == {
        "listened",
        "talked",
    }
...
def test_a_jump_naming_an_undeclared_label_is_refused() -> None:
    ...
    with pytest.raises(ScenarioAdmissionError, match="names undeclared label `nowhere`"):
        _admit(script=script)


def test_a_label_no_path_reaches_is_refused() -> None:
    script = DEFAULT_SCRIPT + "\n\nlabel orphan:\n    end listened\n"
    with pytest.raises(ScenarioAdmissionError, match="labels no path reaches: orphan"):
        _admit(script=script)


def test_an_ending_no_path_reaches_is_refused() -> None:
    endings = [
    ...
    with pytest.raises(ScenarioAdmissionError, match="endings no path reaches: left"):
        _admit(endings=endings)


def test_a_scenario_that_reaches_no_end_is_refused() -> None:
    script = (
        "label start:\n"
        "    stage classroom\n"
        "    show nao neutral\n"
...
def test_a_block_with_no_terminal_statement_is_refused() -> None:
...
def test_a_block_does_not_continue_past_its_terminal_statement() -> None:
    script = DEFAULT_SCRIPT.replace(
        "label ending_quiet:\n    hide nao\n    end listened",
        "label ending_quiet:\n    hide nao\n    end listened\n    hide nao",
    )
...
def test_an_if_run_without_a_default_jump_cannot_compile() -> None:
...
def test_a_flag_no_reachable_set_establishes_is_refused() -> None:
...
def test_a_flag_the_declarations_do_not_carry_is_refused() -> None:
    with pytest.raises(ScenarioAdmissionError, match="undeclared flags: stayed_quiet"):
        _admit(flags=[{"flag_id": "unused_instead"}])


def test_a_declared_flag_the_script_never_uses_is_refused() -> None:
...
def test_a_stage_the_declarations_do_not_carry_is_refused() -> None:
...
def test_a_declared_stage_the_script_never_uses_is_refused() -> None:
    ...
    with pytest.raises(ScenarioAdmissionError, match="stages the script never uses: rooftop"):
        _admit(stages=stages)


def test_an_actor_the_cast_does_not_declare_is_refused() -> None:
...
def test_an_expression_the_actor_does_not_declare_is_refused() -> None:
...
def test_showing_an_actor_that_declares_no_expressions_is_refused() -> None:
...
def test_an_undeclared_outcome_is_refused() -> None:
...
def test_an_undeclared_track_is_refused() -> None:
    script = DEFAULT_SCRIPT.replace(
        "    stage classroom\n", "    stage classroom\n    play cicadas\n"
    )
...
def test_a_cast_member_the_script_never_uses_is_refused() -> None:
    cast = [
        {
            "actor_id": "nao",
...
def test_an_actor_named_after_a_statement_keyword_is_refused() -> None:
...
def _declarations_with_track() -> ScenarioDeclarations:
    ...
    payload["tracks"] = [{"track_id": "room_tone"}]
    return ScenarioDeclarations.model_validate(payload)


def test_playing_an_undeclared_track_is_refused() -> None:
    script = (
        "label start:\n"
        "    stage classroom\n"
        "    play nowhere\n"
    ...
    program = compile_scenario(_declarations_with_track(), parse_scenario(script))
    with pytest.raises(ScenarioAdmissionError, match="undeclared track"):
        admit_scenario(_declarations_with_track(), program)


def test_a_declared_track_nothing_plays_is_refused() -> None:
    # Compatibility admission rejects unused track declarations.
    script = (
        "label start:\n"
        "    stage classroom\n"
        '    you "Nothing plays."\n'
    ...
    program = compile_scenario(_declarations_with_track(), parse_scenario(script))
    with pytest.raises(ScenarioAdmissionError, match="tracks the script never uses"):
        admit_scenario(_declarations_with_track(), program)
