...
from scenario_authoring.compatibility.v2.admission import _apply, _live_flags, _successors
...
def _declarations(**overrides: Any) -> ScenarioDeclarations:
    return ScenarioDeclarations.model_validate(declarations_value(**overrides))
...
def test_an_import_is_enumerated_and_then_dies() -> None:
    ...
    assert report.imported_flags == ["rang_the_bell"]
    live = _live_flags(program)
