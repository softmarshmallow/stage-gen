"""Bounded v2 name, reachability and ending-witness admission.

The search walks `(block, flag assignment)` from each possible initial imported
assignment. Branches take their first satisfied edge, matching the v2 runtime.
Flag liveness projects away values that no subsequent condition can inspect.

Admission refuses undeclared/unused names, unreachable blocks or endings,
nonselectable menus and search-budget overruns. An ending witness is evidence
that the ending can be reached; it is not a guarantee that every possible path
terminates or that every invisible instruction chain settles.
"""
...
def admit_scenario(
    ...
    """Validate references and report bounded reachable states and ending witnesses."""
...
def _check_names(declarations: ScenarioDeclarations, program: ScenarioProgram) -> None:
    ...
    not carry is refused, and so is a declaration nothing uses. This preserves
    the supported v2 authoring admission policy.
    """
    ...
    # An imported flag can be supplied by the invoking game, so nothing
