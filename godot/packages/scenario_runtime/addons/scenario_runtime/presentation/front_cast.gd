...
## Optional front-facing actor binding. The caller supplies textures, geometry,
## controller configuration and time. This node owns only its presentation nodes;
## it never chooses a narrative node, takes input or advances the host world.
## Coordinates are logical screen units with a positive axis-aligned camera.
...
var _handoff_specification: Dictionary = {}
...
func initialize(profiles: Array, textures: Dictionary, manpu_textures: Dictionary, layout: Dictionary) -> Array[String]:
	var errors: Array[String] = _validate_layout(layout)
	var ids: Array[String] = []
	for value: Variant in profiles:
		if not (value is Dictionary) or not ANIMATION.valid_id(value.get("id")):
			errors.append("Front cast requires profiles with valid ids.")
			continue
		var id := String(value["id"])
		if ids.has(id) or not (textures.get(id) is Texture2D):
			errors.append("Front cast requires one prepared texture per unique actor: " + id)
		ids.append(id)
	if ids.is_empty():
		errors.append("Front cast requires at least one actor.")
	...
	var specification: Variant = layout.get("handoff_specification")
	if not (specification is Dictionary) or not (specification.get("slots") is Dictionary):
		errors.append("Front cast requires the prepared Cast Transition slots.")
	...
	for mark_id: Variant in next_marks:
		if not mark_id is String or not next_marks[mark_id] is Texture2D:
			errors.append("Front cast manpu bindings require string ids and Texture2D resources.")
	if not errors.is_empty():
		return errors
	...
	_handoff_specification = specification.duplicate(true)
...
func set_cast(ids: Array) -> Array[String]:
	...
	if ids.size() > int(_layout.get("max_actors", 0)):
		errors.append("Front cast exceeds its configured actor capacity.")
	if not errors.is_empty():
		return errors
	_staged.assign(ids)
	_handoff_ids.clear()
	...
	for index in ids.size():
		var center := float(_layout["single_center"])
		if ids.size() == 2:
			center = float(_slots["left" if index == 0 else "right"])
		elif ids.size() > 2:
			center = lerpf(float(_layout["spread_left"]), float(_layout["spread_right"]), float(index) / float(ids.size() - 1))
...
func focus(actor_id: String, preset_id: String = "bounce", replay: bool = false) -> Array[String]:
	if not actor_id.is_empty() and not _profiles.has(actor_id):
		return ["Unknown front cast focus actor: " + actor_id]
...
func set_marks(cues: Array) -> Array[String]:
	for cue: Variant in cues:
		if not (cue is Dictionary) or not _profiles.has(cue.get("actor")) or not _mark_textures.has(cue.get("id")):
			return ["Front cast manpu cues require a registered actor and raster mark."]
		...
		if cue.get("frames", []) is Array:
			for frame_id: Variant in cue.get("frames", []):
				if not _mark_textures.has(frame_id):
					return ["Front cast manpu frames require prepared raster marks."]
...
func emit_manpu(actor_id: String, mark_id: String, preset_id: String = "sigh_puff") -> Dictionary:
	if not _profiles.has(actor_id) or not _mark_textures.has(mark_id) or not visible_ids().has(actor_id):
		return {"errors": ["Front cast one-shot manpu requires a visible actor and prepared raster mark."], "instance_id": -1}
...
func handoff(outgoing: String, survivor: String, incoming: String, settings: Dictionary = {}) -> Array[String]:
	...
	if is_busy() or visible_ids() != [outgoing, survivor]:
		errors.append("Front cast handoff requires an idle outgoing/survivor pair in left/right order.")
	if not errors.is_empty():
		return errors
	if not is_equal_approx(float(_positions[outgoing]), float(_slots["left"])) or not is_equal_approx(float(_positions[survivor]), float(_slots["right"])):
		return ["Front cast handoff requires the authored two-person slot positions."]
	var next_cast = CAST.new()
	errors.append_array(next_cast.initialize(ids, EXIT_CATALOG, _handoff_specification))
...
func dismiss(actor_id: String, preset_id: String = "silhouette_fade") -> Array[String]:
	if not _staged.has(actor_id) or not _handoff_ids.is_empty():
		return ["Front cast dismissal requires a staged actor outside a cast handoff."]
...
func move_actor(actor_id: String, center_x: float, settings: Dictionary = {}) -> Array[String]:
	...
	if not visible_ids().has(actor_id) or _exit.is_exiting(actor_id) or not _handoff_ids.is_empty():
		errors.append("Front cast movement requires a visible actor outside an exit or cast handoff.")
	if bool(_movement.get("active", false)) and _movement.get("actor") != actor_id:
		errors.append("Front cast blocking supports one active moving actor.")
	if not is_finite(center_x) or center_x < 0.0 or center_x > float(_layout["width"]):
		errors.append("Front cast movement center_x must be finite and inside the configured stage width.")
	for key: Variant in settings:
		if not MOVE_DEFAULTS.has(key):
			errors.append("Unknown front cast movement setting: " + str(key))
	...
	if not ANIMATION.finite_number(configured["duration_seconds"]) or float(configured["duration_seconds"]) < 0.0 or float(configured["duration_seconds"]) > 5.0:
		errors.append("Front cast movement duration_seconds must be finite and between 0 and 5.")
...
func approach_actor(actor_id: String, target_id: String, settings: Dictionary = {}) -> Array[String]:
	if actor_id == target_id or not visible_ids().has(actor_id) or not visible_ids().has(target_id) or _exit.is_exiting(target_id):
		return ["Front cast approach requires distinct visible actors and a stationary target outside an exit."]
	...
	if not ANIMATION.finite_number(gap) or float(gap) <= 0.0 or float(gap) >= absf(target_x - from_x):
		return ["Front cast approach stop_distance must be finite, positive, and smaller than the current separation."]
...
func set_projection(actor_id: String, enabled: bool) -> Array[String]:
	if not _profiles.has(actor_id):
		return ["Unknown front cast projection actor: " + actor_id]
...
func advance(delta: float) -> void:
	...
	if bool(_movement.get("active", false)):
		...
		_positions[_movement["actor"]] = clampf(lerpf(float(_movement["from_x"]), float(_movement["target_x"]), weight), 0.0, float(_layout["width"]))
...
func get_actor_rect(actor_id: String) -> Rect2:
	...
	var dimensions := Vector2(float(_layout["actor_height"]) * texture.get_width() / texture.get_height(), float(_layout["actor_height"]))
	var center := float(_appearance(actor_id)["center_x"])
	return Rect2(Vector2(center - dimensions.x * 0.5, float(_layout["actor_top"])), dimensions)
...
func _present_mark(node: TextureRect, actor_id: String, mark_id: String, sample: Dictionary, camera: Transform2D) -> void:
	...
	var offset: Array = _layout["mark_offset"]
	var bounds: Array = _layout["mark_bounds"]
	var anchor := Vector2(clampf(float(eye[0]) + float(offset[0]), float(bounds[0]), float(bounds[2])), maxf(float(bounds[1]), float(eye[1]) + float(offset[1])))
	if _layout.get("mark_y_offsets", {}).has(mark_id):
		anchor.y = float(eye[1]) + float(_layout["mark_y_offsets"][mark_id])
	var center := owner.position + owner.size * anchor
	var height := owner.size.y * float(_layout["mark_height_ratio"])
...
func _appearance(actor_id: String) -> Dictionary:
	...
	result["center_x"] = float(_positions.get(actor_id, _layout["single_center"]))
	return result
...
func _validate_ids(ids: Array) -> Array[String]:
	...
	for id: Variant in ids:
		if not (id is String) or not _profiles.has(id) or seen.has(id):
			errors.append("Unknown or duplicate Front cast actor: " + str(id))
		seen[id] = true
	return errors


func _validate_layout(layout: Dictionary) -> Array[String]:
	var errors: Array[String] = []
	for key in ["width", "actor_height", "actor_top", "single_center", "spread_left", "spread_right", "mark_height_ratio"]:
		if not ANIMATION.finite_number(layout.get(key)):
			errors.append("Front cast layout requires a finite " + key + ".")
	if not errors.is_empty(): return errors
	if float(layout["width"]) <= 0.0 or float(layout["actor_height"]) <= 0.0 or float(layout["mark_height_ratio"]) <= 0.0:
		errors.append("Front cast dimensions and mark ratio must be positive.")
	if not layout.get("max_actors") is int or int(layout.get("max_actors", 0)) < 1:
		errors.append("Front cast capacity must be a positive integer.")
	for key in ["single_center", "spread_left", "spread_right"]:
		if float(layout[key]) < 0.0 or float(layout[key]) > float(layout["width"]):
			errors.append("Front cast placement must fit its stage width.")
	for pair in [["mark_offset", 2], ["mark_bounds", 3]]:
		var value: Variant = layout.get(pair[0])
		if not value is Array or value.size() != pair[1]:
			errors.append("Front cast " + pair[0] + " requires its coordinate array.")
			continue
		for number: Variant in value:
			if not ANIMATION.finite_number(number): errors.append("Front cast mark coordinates must be finite.")
	if not layout.get("handoff_specification") is Dictionary:
		errors.append("Front cast requires a handoff specification.")
	return errors
