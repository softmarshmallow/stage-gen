#!/usr/bin/env python3
"""Assemble templates/vn and its declared Scenario dependencies offline."""
...
SDK_PATH = Path("addons/scenario_runtime")
...
def assemble(output: Path, package: Path = PACKAGE, template: Path = TEMPLATE) -> dict[str, object]:
    ...
    with staged_destination(destination) as temporary:
        ...
        report: dict[str, object] = {
            "schema_version": 1,
            "kind": "scenario_starter",
            "sdk_path": SDK_PATH.as_posix(),
            "sdk_files": copied.pop("scenario_runtime"),
