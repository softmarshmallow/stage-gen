...
func resolve(relative_path: String) -> Dictionary:
	...
	if not errors.is_empty(): return {"path": "", "errors": errors}
	if not FileAccess.file_exists(path) and not (_backend == "resources" and ResourceLoader.exists(path)):
...
func read_bytes(relative_path: String) -> Dictionary:
	...
	return {"bytes": file.get_buffer(file.get_length()), "errors": []}
...
func load_texture(relative_path: String, mipmaps: bool = false) -> Dictionary:
	...
	return {"resource": ImageTexture.create_from_image(picture), "errors": []}
