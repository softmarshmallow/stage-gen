...
when it appears. [manpu_animation.gd](manpu_animation.gd) owns one independent
clock per persistent `(actor, id)` pair and uses the shared
[Presentation Animation sampler](../motion/ANIMATION.md). Actor Focus owns the
...
[presets.json](presets/manpu.json) follows the common version-1 catalog with one
...
[`qa/one_shot_manpu_checks.gd`](../../../tests/one_shot_manpu_checks.gd). It covers
...
See the [request archive](../../../history/USER_PROMPTS.md#p21) for the user's wording and
[QA record](../../../history/QA.md) for validation and rendered evidence.
...
[manpu_loop_checks.gd](../../../../../games/command_link/tests/manpu_loop_checks.gd) checks held-step boundaries,
