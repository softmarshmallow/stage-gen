...
[P22](../../../history/USER_PROMPTS.md#p22); **Opacity Fade** supplies a direct comparison. P69 adds **Walk-Away** with left/right presets.
These are local project terms, with no claim of universal industry taxonomy
or production-module promotion.

[character_exit.gd](character_exit.gd) owns an independent
`visible → exiting → hidden` lifecycle for each registered actor. It uses the
[Presentation Animation sampler](../motion/ANIMATION.md); the stage owns the
...
The two fade presets in [presets.json](presets/exit.json) last 0.9 seconds and use the common
...
[QA.md](../../../history/QA.md) records verification and visual evidence separately from this
contract.
