...
## P117 split (2026-09-11)

The workspace no longer exists. Afterlight is `godot/games/afterlight/` and Command Link is
`godot/games/command_link/`, each its own project with its own Lab, tests and tools; the
starter is `godot/templates/vn/`; this history and the SDK's pure suites live in
`godot/packages/game_presentation/`. Launch: `Godot --path godot/games/afterlight -- --language ko`.
Paths in the sections below describe the workspace as it was.

## P116 move (2026-09-11)
...
- Development Godot project: `godot/games/playground/`. The directory name and
  launch commands are retained; it is no longer under ignored `spikes/`.
- Installable SDK: `godot/games/playground/addons/game_presentation/`, installed
...
3. [Packaging and external content](../docs/PACKAGING.md),
   [starter](../../../templates/vn/README.md), and
...
5. [Terminology dictionary](../docs/TERMINOLOGY.md) for effect names and example use cases.
...
/Users/universe/.local/bin/Godot --path /Users/universe/Documents/shared/stage-gen/godot/games/playground -- --game bishoujo_afterlight --language ko --content-root /private/tmp/game-presentation-content-p107
```

From the repository root, assemble a fresh starter/content directory if needed:

```sh
python3 godot/games/playground/tools/assemble_starter.py --output /private/tmp/my-next-vn
python3 godot/games/playground/tools/prepare_example_content.py --output /private/tmp/my-next-content
...
.venv/bin/python -m pytest godot/games/playground/qa/python/test_starter_assembly.py godot/games/playground/qa/python/test_example_content.py -q
