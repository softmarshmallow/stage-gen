...
| Presentation Animation | Shared scalar keyframe sampling for position offset, scale, opacity, and brightness. Target controllers own their cues and clocks; see the [common local contract](../addons/game_presentation/motion/ANIMATION.md). |
| Actor Focus | Working umbrella for speaker emphasis, triggered by a change of speaking actor. Its interchangeable presets include motion, enlargement, dimming, fading, and none; see the [local contract](../addons/game_presentation/actors/ACTOR_FOCUS.md). |
| Manpu Introduction | A local animation for each newly visible actor/mark pair, composed with its owner's motion. It uses the shared sampler with independent clocks; see the [local contract](../addons/game_presentation/actors/MANPU.md). |
...
| Actor Halo / Outer Glow | A soft colored outline outside a sprite's alpha silhouette, rendered behind the unchanged source. Its [contract](../addons/game_presentation/effects/ACTOR_HALO.md) owns source geometry, padding, color, radius, intensity, and strength. |
| Ominous Corruption | A screen-reading treatment with darkening, mist/aura, motes, mild refraction, and red glow. An actor's alpha can define coverage, but areas or the full screen also work. [Contract](../addons/game_presentation/effects/SCREEN_FIELDS.md). |
...
| Character Exit | Working umbrella for a per-actor departure lifecycle and its interchangeable color/coverage presets; see the [local contract](../addons/game_presentation/actors/CHARACTER_EXIT.md). |
| Cast Transition | Working umbrella for a sequential cast handoff: departure, optional survivor repositioning, then arrival. The pattern owns this order; see the [local contract](../addons/game_presentation/actors/CAST_TRANSITIONS.md). |
...
embedded in a character shader or in TTS generation. The [processor contract](../addons/game_presentation/audio/VOICE_PROCESSING.md)
...
listening quality is unreviewed. See [audio evidence](../../../games/afterlight/tests/transmission-audio/REVIEW.md).

## Request and evidence

The user's exact wording is [P16](../history/USER_PROMPTS.md#p16); the short outcome belongs
in [REQUESTS.md](../history/REQUESTS.md). [README.md](../history/PLAYGROUND_README.md) describes the playable
controls and [QA.md](../history/QA.md) records validation and visual evidence.
